...
EkaFhRunGroup::EkaFhRunGroup(EfhCtx *pEfhCtx,
  ...
  udpCh = new EkaUdpChannel(dev, dev->snDev->dev_id, coreId,
                            -1);
  if (!udpCh)
    on_error("!udpCh");
  ...
  auto gr0 = fh->b_gr[groupList[0]];

  EKA_LOG("%s: coreId = %u, runId = %u, udpChId = %d, "
          "epmRegionId = %d, MC groups: %s, "
          "No MD check time frame: %02d:%02d -- %02d:%02d",
          EKA_EXCH_DECODE(exch), coreId, runId, udpChId,
          epmRegionId, list2print,

          gr0->mdCheckStartHour, gr0->mdCheckStartMinute,
          gr0->mdCheckEndHour, gr0->mdCheckEndMinute);
}

void EkaFhRunGroup::setTimeToCheckNoMdFlag() {
  auto gr0 = fh->b_gr[groupList[0]];
  checkNoMd = isTradingHours(
      gr0->mdCheckStartHour, gr0->mdCheckStartMinute,
      gr0->mdCheckEndHour, gr0->mdCheckEndMinute);
}
/* ###################################################### */
bool EkaFhRunGroup::igmpSanityCheck(int grId2check,
  ...
  for (auto i = 0; i < 512; i++) {
    int chId = EkaEpmRegion::getEfhIgmpRegion(
        hwIgmp[i].channel - 32);
    if ((hwIgmp[i].group_address != 0) &&
        (chId < 0 || chId > 31))
      on_error("chId=%d,hwIgmp[%d].channel=%u", chId, i,
               hwIgmp[i].channel);
    if (hwIgmp[i].group_address == 0)
      continue;
    ...
    if (grId < 0 || grId > 63)
      on_error(
          "chId=%d,grId=%d,hwIgmp[%d].positionIndex=%u,"
          "group_address=0x%08x",
          chId, grId, i, hwIgmp[i].positionIndex,
          hwIgmp[i].group_address);

    EKA_LOG(
        "%3d (%3d): pos=%3u, lane=%u, ch=%2u (%d), %s:%u",
        i, grId, hwIgmp[i].positionIndex, hwIgmp[i].lane,
        hwIgmp[i].channel, chId,
        EKA_IP2STR(be32toh(hwIgmp[i].group_address)),
        hwIgmp[i].ip_port_number);

    mcState.chState[chId].grpHwState[grId].coreId =
        hwIgmp[i].lane;
    mcState.chState[chId].grpHwState[grId].ip =
        be32toh(hwIgmp[i].group_address);
    mcState.chState[chId].grpHwState[grId].port =
        hwIgmp[i].ip_port_number;
    mcState.chState[chId].grpHwState[grId].positionIndex =
        hwIgmp[i].positionIndex;
    mcState.chState[chId].currHwGr++;
  }

  if (coreId != mcState.chState[epmRegionId]
                    .grpHwState[grId2check]
                    .coreId) {
    EKA_WARN("WARNING: chId %d, grId %d: "
             "expected coreId %d != actual %d",
             epmRegionId, grId2check, coreId,
             mcState.chState[epmRegionId]
                 .grpHwState[grId2check]
                 .coreId);
    return false;
  }

  if (ip != mcState.chState[epmRegionId]
                .grpHwState[grId2check]
                .ip ||
      port != mcState.chState[epmRegionId]
                  .grpHwState[grId2check]
                  .port) {
    EKA_WARN("WARNING: chId %d, grId %d: "
             "expected %s:%u != actual %s:%u",
             epmRegionId, grId2check, EKA_IP2STR(ip), port,
             EKA_IP2STR(mcState.chState[epmRegionId]
                            .grpHwState[grId2check]
                            .ip),
             mcState.chState[epmRegionId]
                 .grpHwState[grId2check]
...
void EkaFhRunGroup::checkGroupsNoMd(
    const EfhRunCtx *pEfhRunCtx) {
  for (auto i = 0; i < numGr; i++) {
...
/* #####################################################################
 */
void EkaFhRunGroup::invalidateAllGroups(
    const EfhRunCtx *pEfhRunCtx) {
  for (auto i = 0; i < numGr; i++) {
    EkaFhGroup *gr = fh->b_gr[groupList[i]];
    if (!gr)
...
int EkaFhRunGroup::sendFeedCloseAll(
    const EfhRunCtx *pEfhRunCtx) {
  for (auto i = 0; i < numGr; i++) {
    EkaFhGroup *gr = fh->b_gr[groupList[i]];
    if (!gr) {
...
void EkaFhRunGroup::setGrAfterGap(uint i) {
  if (cntGrAfterGap > numGr)
    on_error("cntGrAfterGap %u > numGr %u", cntGrAfterGap,
             numGr);
...
EkaFhRunGroup::~EkaFhRunGroup() {
  ...
  EKA_LOG(
      "Waiting for all groups sending FeedDownClosed...");
