...
struct EkaOsiSymbolData {
  ...
  constexpr uint32_t getYYYYMMDD() {
    return static_cast<uint32_t>(year) * 10000 +
  ...
  constexpr void
  copyRoot(char (&dest)[N],
           const char (&symbol)[SymbolLength]) {
    static_assert(
        N > RootSize,
        "destination is too small to hold the largest root "
        "name (6 chars) + a terminator");
    std::memcpy(dest, &symbol[RootStart], rootLength);
    std::memset(dest + rootLength, '\0', N - rootLength);
  }

  constexpr bool
  parseFromSymbol(const char (&symbol)[SymbolLength]) {
    constexpr uint8_t MaxDaysInMonth[12] = {
    ...
    if (foundRootLength < 1)
      return false;

    for (std::size_t i = ExpiryStart;
         i < ExpiryStart + ExpirySize; i++) {
    ...
    uint16_t y =
        expiryDigit(0) * 10 + expiryDigit(1) + 2000;
    uint8_t m = expiryDigit(2) * 10 + expiryDigit(3);
    uint8_t d = expiryDigit(4) * 10 + expiryDigit(5);
    if (m < 1 || m > 12 || d < 1 ||
        d > MaxDaysInMonth[m - 1]) {
      return false;
    }

    if (symbol[OptionTypePos] != 'P' &&
        symbol[OptionTypePos] != 'C') {
      return false;
    }

    int64_t osiStrikePrice;
    if (!parseFullStrToNum(&symbol[StrikePriceStart],
                           StrikePriceSize,
                           &osiStrikePrice)) {
      return false;
    }
    ...
    this->month = m;
    this->day = d;
    this->optionType = symbol[OptionTypePos] == 'C'
                           ? EfhOptionType::kCall
                           : EfhOptionType::kPut;
    this->efhStrikePrice = priceToEfhScale(
        osiStrikePrice, StrikePriceDecimalPlaces);
