...
/* ############################################## */
const uint8_t* EkaFhNasdaq::getUdpPkt(EkaFhRunGroup* runGr,
																			uint* msgInPkt,
																			uint64_t* sequence,
																			uint8_t* gr_id) {
  ...
  if (grId == 0xFF ||
			...
			b_gr[grId]->q == NULL) { 
    if (grId != 0)
			EKA_LOG("RunGr%u: Skipping gr_id = %u not belonging to %s",
							runGr->runId,grId,runGr->list2print);
    runGr->udpCh->next(); 
    return NULL;
  }
  auto gr = dynamic_cast<EkaFhNasdaqGr*>(b_gr[grId]);
  if (!gr)
		on_error("gr[%u ] == NULL",grId);

  if (gr->firstPkt) {
    memcpy((uint8_t*)gr->session_id,
					 ((const mold_hdr*)pkt)->session_id,10);
    gr->firstPkt = false;
    EKA_LOG("%s:%u session_id is set to %s",
						EKA_EXCH_DECODE(exch),grId,
						(char*)gr->session_id + '\0');
  }
  *msgInPkt = msgCnt;
...
/* ############################################## */
EkaOpResult EkaFhNasdaq::runGroups( EfhCtx* pEfhCtx,
  ...
  while (runGr->thread_active && ! runGr->stoppedByExchange) {
    ...
    if (gr->state == EkaFhGroup::GrpState::NORMAL && 
      ...
      EKA_WARN("%s:%u: TEST GAP INJECTED: "
							 ...
							 "pkt sequence %ju with %u messages dropped",
    ...
    case EkaFhGroup::GrpState::INIT : {
      ...
      if (gr->skipSnapshot()) {
				...
				EKA_LOG("%s:%u Skipping Snapshot for pure "
								"Trade/Auction group: sequence=%ju",
								EKA_EXCH_DECODE(exch),gr_id,sequence);
