...
public:
  ...
  uint64_t getDataTemplateAddr() {
    return EPM_DATA_TEMPLATE_BASE_ADDR +
           id * EpmMaxRawTcpSize;
  }

  uint64_t getCsumTemplateAddr() {
    return EPM_CSUM_TEMPLATE_BASE_ADDR +
           id * (2 /*msb+lsb*/ * EpmNumHwFields *
                 EpmHwFieldSize / 8 /*1bit per value*/);
  }

  uint getByteSize() const {
    return (uint16_t)(byteSize - sizeof(EkaEthHdr) -
                      sizeof(EkaIpHdr) - sizeof(EkaTcpHdr));
  }

  virtual uint16_t payloadSize() {
    return (uint16_t)(EpmMaxRawTcpSize - sizeof(EkaEthHdr) -
                      sizeof(EkaIpHdr) - sizeof(EkaTcpHdr));
  }
  ...
  inline void setCsBitmap(uint idx, uint8_t hwIdx) {
    ...
    if (idx % 2 == 0) {
      // hw_tcpcs_template.low.row[row].bitmap |=
      // ((uint16_t)1)<<col;
      uint16_t bitmapTmp =
          hw_tcpcs_template.low.row[row].bitmap | bitmapSet;
      hw_tcpcs_template.low.row[row].bitmap = bitmapTmp;
    } else {
      // hw_tcpcs_template.high.row[row].bitmap |=
      // ((uint16_t)1)<<col;
      uint16_t bitmapTmp =
          hw_tcpcs_template.high.row[row].bitmap |
          bitmapSet;
