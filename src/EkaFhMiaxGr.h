...
public:
  ...
  int subscribeStaticSecurity(
    ...
    if (book == NULL)
      on_error("%s:%u book == NULL", EKA_EXCH_DECODE(exch),
               id);
...
private:
  inline auto findUnderlying(const char *name) {
    for (int i = 0; i < (int)underlyingNum; i++) {
      /* TEST_LOG("Comparing \'%s\' vs
       * \'%s\'",name,underlying[i]->name); */
      /* if (memcmp(&underlying[i]->name,name,size) == 0)
       * return i; */
      /* TEST_LOG("memcmp - FALSE"); */
      if (strcmp(underlying[i]->name, name) == 0)
        return i;
  ...
  inline auto addUnderlying(const char *name) {
    ...
    if (underlyingNum == UndelyingsPerGroup)
      on_error("cannot add %s to gr %u: underlyingNum=%u",
               name, id, underlyingNum);

    uint newUnderlying = underlyingNum++;

    underlying[newUnderlying] = new Underlying(name);
    if (underlying[newUnderlying] == NULL)
      on_error("cannot create new Underlying %s", name);
#if 0
    char name2print[16] = {};
...
public:
  ...
  using FhBook = EkaFhTobBook<
      SEC_HASH_SCALE,
      EkaFhTobSecurity<SecurityIdT, PriceT, SizeT>,
      EkaFhNoopQuotePostProc, SecurityIdT, PriceT, SizeT>;
