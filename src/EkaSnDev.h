...
class EkaSnDev {
 public:
  EkaSnDev(EkaDev* _dev) {
    ...
    if ((dev_id = SN_OpenDevice(NULL, NULL)) == NULL)
...
//################################################
  ...
  inline uint64_t read(uint64_t addr) {
    uint64_t ret;
    if (SN_ERR_SUCCESS != SN_ReadUserLogicRegister(dev_id, addr/8,
																									 &ret))
...
//################################################
  ...
  inline void write(uint64_t addr, uint64_t val) {
    if (SN_ERR_SUCCESS != SN_WriteUserLogicRegister(dev_id,
																										addr/8, val))
...
//################################################
  ...
  void reset() {
    ...
    if (SN_ResetFPGA(dev_id) != SN_ERR_SUCCESS )
			on_error("Failed to reset FPGA");    
  }
//################################################

  bool       getLinkStatus(uint8_t lane, bool* sfp_present,
													 bool* link) {
    SN_StatusInfo statusInfo;
    int rc = SN_GetStatusInfo(dev_id, &statusInfo);
    if (rc != SN_ERR_SUCCESS)
			on_error("Failed to get status info, error code %d",rc);
...
//################################################
  ...
  bool       hasLink(uint8_t lane) {
    ...
    if (rc != SN_ERR_SUCCESS)
			on_error("Failed to get status info, error code %d",rc);
    SN_LinkStatus *s = (SN_LinkStatus *) &statusInfo.LinkStatus[lane];
    EKA_LOG("Core %u: sfp = %d, link = %d",
						lane,s->SFP_Present, s->Link);
...
//################################################
  ...
  void       getIpMac(uint8_t lane, uint32_t* ip, uint8_t* mac) {
    if (SC_GetLocalAddress(dev_id,lane,
													 (struct in_addr*) ip,
													 NULL, NULL, mac) != SC_ERR_SUCCESS)
			on_error("Error on SC_GetLocalAddress");
    in_addr ip2set = {*ip};
    in_addr netmask2set = {};
    in_addr gw2set = {};

    if (SC_SetLocalAddress(dev_id,lane, ip2set, netmask2set,
													 gw2set, mac) != SC_ERR_SUCCESS) 
...
//################################################
  void       getIpMac_ioctl(uint8_t lane, uint32_t* ip,
														uint8_t* mac) {
#if 1
		char          buf[1024];
		struct ifconf ifc;
		...
		for(int i = 0; i < nInterfaces; i++){
			...
			if (strncmp(item->ifr_name,portName.c_str(),
									strlen(portName.c_str())) != 0)
				continue;
...
#else
    in_addr gw, nm;
    if (SN_GetLocalAddress(dev_id,lane, (in_addr*) ip,
													 &gw, &nm, mac) != SN_ERR_SUCCESS)
			on_error("Error on SN_GetLocalAddress");
    if (*(uint32_t*)ip == 0)
			on_error("ip = 0");
#endif
    return;
...
//################################################
  ...
  void set_fast_session (uint8_t core, uint8_t sess,
												 uint32_t srcIp, uint16_t srcUdp,
												 uint32_t dstIp, uint16_t dstUdp) {
    EKA_LOG("core=%u, sess=%u %s:%hu --> %s:%hu",
						core,sess,EKA_IP2STR(srcIp),srcUdp,
						EKA_IP2STR(dstIp),dstUdp);
