...
public:
  ...
  inline void epmCopyWcBuf(uint64_t hwHeapAddr,
                           ...
                           uint16_t epmRegion) {
    int wcChId = EkaEpmRegion::geWcChByOffs(hwHeapAddr);

    while (busy_[wcChId].test_and_set(
        std::memory_order_acquire)) {
      busy_[wcChId].test(std::memory_order_relaxed);
    }
    auto wcRegionBase =
        (volatile uint64_t *)((uint64_t)snDevWCPtr_ +
                              wcChId * 0x800);

    if ((uint64_t)wcRegionBase - (uint64_t)snDevWCPtr_ !=
        wcChId * 0x800)
    ...
    epmCopyWcDesc(wcRegionBase, pktSize, hwHeapAddr,
                  ...
                  0, // tcpPseudoCsum,
                  SendOp::DontSend);
    epmCopyWcPayload(wcRegionBase + 8, swHeapAddr,
                     roundUp64(pktSize));

    busy_[wcChId].clear(std::memory_order_release);
  ...
  inline void epmCopyAndSendWcBuf(const void *swHeapAddr,
    ...
    epmCopyWcDesc(wcRegionBase, pktSize, hwHeapAddr,
                  actionLocalIdx, epmRegion, tcpPseudoCsum,
                  SendOp::Send);
    epmCopyWcPayload(wcRegionBase + 8, swHeapAddr,
                     roundUp64(pktSize));
...
private:
  ...
  static const size_t MaxWcCh = 16;
  ...
  inline void
  epmCopyWcDesc(volatile uint64_t *descAddr,
                const size_t pktSize, uint64_t hwHeapAddr,
                uint16_t actionLocalIdx, uint16_t epmRegion,
                uint32_t tcpPseudoCsum, SendOp send) {
    struct WcDesc {
    ...
    WcDesc __attribute__((aligned(0x100))) desc = {
        .trigDesc = {.str = {.tcp_cs = tcpPseudoCsum,
                             .action_index = actionLocalIdx,
                             .size = (uint16_t)(pktSize &
                                                0xFFFF),
                             .region = epmRegion}},
    ...
    EKA_LOG("desc: action_index=%u, hwHeapAddr=%x, size=%u, nBytes=%u, opc=%u",
	     ...
	     desc.opc);
#endif
    atomicCopy((volatile uint64_t *)descAddr, &desc,
               sizeof(desc));
    return;
  }
