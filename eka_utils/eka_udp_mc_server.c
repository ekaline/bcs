...
void print_usage(char* cmd) {
  printf ("USAGE: %s -m [MC IP] -p [MC Port] -i [Local IP to bind]\n",cmd);
  exit (1);    
}
...
int main(int argc, char *argv[]) {
  ...
  if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEPORT, &const_one, sizeof(int)) < 0) 
  ...
  if (bind(sock_fd,(struct sockaddr*) &local2bind, sizeof(struct sockaddr)) < 0)
  ...
  TEST_LOG ("joining IGMP %s from local addr %s ",EKA_IP2STR(mreq.imr_multiaddr.s_addr),EKA_IP2STR(mreq.imr_interface.s_addr));

  if (setsockopt(sock_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0)
    on_error ("fail to join IGMP  %s:%d\n",inet_ntoa(mreq.imr_multiaddr),be16toh(local2bind.sin_port));
