...
void  INThandler(int sig) {
  signal(sig, SIG_IGN);
  keep_work = false;
  fprintf(stderr,"Ctrl-C detected: keep_work = false, exitting..."); fflush(stdout);fflush(stderr);
  return;
}
...
void* Child(void* arg) {
  ...
  do {
    char rxBuf[2000] = {};
    ssize_t bytes_read = recv(client, rxBuf, sizeof(rxBuf), 0);
    if (bytes_read > 0) {
...
int main(int argc, char *argv[]) {
  if (argc != 2)
    on_error("TCP server port not specified: argc=%d",argc);
