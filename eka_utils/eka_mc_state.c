...
struct McChState {
  ...
  int          currHwGr       = 0;
  EkaMcState   grpState[64]   = {}; // from FPGA scratchpad 
  EkaHwMcState grpHwState[64] = {}; // ioctl() from driver
};
...
int printMcGroups(McState* state) {
  ...
  for (auto chId = 0; chId < 32; chId++) {
		auto ch = &state->chState[chId];
		if (!ch)
			on_error("!ch");
    if (ch->num == 0)
			continue;
    printf("=======================\n"
					 "Ch%2d : Core %d, MC groups %2d\n",
					 chId,
					 ch->grpState[0].coreId,
					 ch->num);

		for (auto grId = 0; grId < ch->num; grId++) {
			
#define _LINE_FORMAT "%2d %16s:%5u (%d %3d %16s:%5u) %9ju\n"
			const char* coloredFmt = 
				ch->grpState[grId].ip   == ch->grpHwState[grId].ip &&
				ch->grpState[grId].port == ch->grpHwState[grId].port ?
				_LINE_FORMAT : RED _LINE_FORMAT RESET;
#undef _LINE_FORMAT

			printf (coloredFmt,
							grId,
							EKA_IP2STR(ch->grpState[grId].ip),
							ch->grpState[grId].port,
							ch->grpHwState[grId].coreId,
							ch->grpHwState[grId].positionIndex,
							EKA_IP2STR(ch->grpHwState[grId].ip),
							ch->grpHwState[grId].port,
							ch->grpState[grId].pktCnt
							);
		} // groups
