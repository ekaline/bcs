...
int main(int argc, char *argv[])
{
  ...
  if ( argc < 2 )
    {
      printf("Wrong usage, need to supply filename with secids (use -f flag)\n");
      return 1;
    }
  
  //  char short_options[] = "f:a";
  
  //  int c = getopt(argc, argv, short_options);
  
  subscriptions_file = fopen(optarg, "r");
	  ...
	  if (found) {
	    ...
	    printf("bid_min_price: %d (0x%x) | "
		   ...
		   c->lower_bytes_of_sec_id, c->lower_bytes_of_sec_id,
	           (c->lower_bytes_of_sec_id == (user_sec_id & 0xFF) ) ? "ok" : "wrong"
		   );
