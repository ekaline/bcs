...
inline void printCommonDef(const EfhSecurityDef* h) {
  ...
  printf("%s,",std::string(h->classSymbol,sizeof(h->classSymbol)).c_str());
  printf("%s,",std::string(h->underlying,sizeof(h->underlying)).c_str());
  printf("%s,",std::string(h->exchSecurityName,sizeof(h->exchSecurityName)).c_str());
...
inline void printOptionDefinition(const EfhOptionDefinitionMsg* msg) {
  printHdr(&msg->header);
  printCommonDef(&msg->commonDef);
  printf("%jd,",msg->strikePrice);
  printf("%c,",msg->optionType == EfhOptionType::kCall ? 'C' : 'P');
  printf("\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (!definitionsFile)
    on_error("cannot open %s file",argv[1]);
