...
Order* EkaNyseBookSide::findOrder(OrderId orderId) {
...
/* ----------------------------------------------------- */
inline int EkaNyseBookSide::addOrder2Hash(Order* o, OrderId _id) {      
  uint16_t hashIdx = calcOrderHash(_id);
  Order** o_ptr = &ordHashTable[hashIdx];
  for (; *o_ptr != NULL; o_ptr = &((*o_ptr)->next))
    if (unlikely((*o_ptr)->getId()== _id)) on_error("Adding existing order %u",_id);
...
//inline int EkaNyseBookSide::addOrder(OrderId _id, Price price, Size _size) {
int EkaNyseBookSide::addOrder(OrderId _id, Price price, Size _size) {
  if (findOrder(_id) != NULL) on_error("adding existing OrderId %u ",_id);

  if (unlikely(++ordersInUse == ORDERS_FREE_LIST_SIZE)) 
    on_error("Out of %u preallocated orders",ORDERS_FREE_LIST_SIZE);
...
inline int EkaNyseBookSide::freeOrder(Order* o) {
...
inline Order* EkaNyseBookSide::getNewFreeOrder() {
...
inline int EkaNyseBookSide::freePLevel(PriceLevel* p) {
...
inline PriceLevel* EkaNyseBookSide::getNewFreePlevel() {
  if (unlikely(freePriceLevels == NULL)) on_error("freePriceLevels = NULL");
...
inline PriceLevel* EkaNyseBookSide::addPlevelAfterTob(Price _price) {
...
inline PriceLevel* EkaNyseBookSide::addPlevelAfterP(PriceLevel* p, Price _price) {
  PriceLevel* newP = getNewFreePlevel();
  if (newP == NULL) on_error("newP == NULL");
  newP->setPrice(_price);
...
inline PriceLevel* EkaNyseBookSide::findOrAddPlevel(Price _price) {
...
inline int EkaNyseBookSide::deletePLevel(PriceLevel* p) {
  if (p->isTop()) {
...
inline int EkaNyseBookSide::removeOrderFromPLevel(PriceLevel* p, Size _size) {
  p->removeOrder(_size);
...
inline int EkaNyseBookSide::removeOrderFromHash(Order* o2rm) {
  ...
  on_error("Order %u not found at Hash",o2rm->getId());
}

/* ----------------------------------------------------- */
 
int EkaNyseBookSide::modifyOrder(OrderId _id, Price _price, Size _size) {
...
int EkaNyseBookSide::deleteOrder(Order* o) {
  if (o == NULL) return 1;
...
/* ----------------------------------------------------- */
inline PriceLevel* EkaNyseBookSide::getPlevelByIdx(uint idx) {
  PriceLevel* p = tob;
  uint cnt = 0;
...
//inline EkaNyseBookSide::Price EkaNyseBookSide::getEntryPrice(uint idx) {
Price EkaNyseBookSide::getEntryPrice(uint idx) {
...
//inline uint EkaNyseBookSide::getEntrySize(uint idx) {
uint EkaNyseBookSide::getEntrySize(uint idx) {
  PriceLevel* p = getPlevelByIdx(idx);
  if (p == NULL) return 0;
  return p->getSize();
}
...
template <class T> uint64_t EkaNyseBookSide::countElems(T* head) { 
  T* ptr = head;
  uint64_t cnt = 0;
  while (ptr != NULL) {
    cnt++;
...
/* ----------------------------------------------------- */
inline uint64_t EkaNyseBookSide::countOrders() {
  uint64_t cnt = 0;
  for (uint i = 0; i < HASH_TABLE_SIZE; i++) {
    for (Order* o = ordHashTable[i]; o != NULL; o = o->next) {
      if (o->getPlevel() == NULL) on_error("%u: o->getPlevel() == NULL, i = 0x%x",o->getId(),i);
      cnt++;
    }
  }
  return cnt;
...
void EkaNyseBookSide::printSide() { 
  printf("%s :\n", side == BID ? "BID" : "ASK");
...
/* ----------------------------------------------------- */
inline bool EkaNyseBookSide::checkPlevels() {
...
inline bool EkaNyseBookSide::checkOrders() {
...
inline bool EkaNyseBookSide::checkPlevelsCorrectness() {
  bool res = true;
  for (PriceLevel* p = tob; p != NULL; p = p->next) {
    uint cnt = 0;
    uint size = 0;
...
/* ----------------------------------------------------- */
//inline bool EkaNyseBookSide::checkIntegrity() { 
bool EkaNyseBookSide::checkIntegrity() { 
...
void EkaNyseBookSide::printStats() {
  EKA_LOG("%s: maxOrdersInUse = %u, maxPLevelsInUse = %u",
	  side == BID ? "BID" : "ASK",maxOrdersInUse,maxPLevelsInUse);
}
/* ----------------------------------------------------- */

EkaNyseBookSide::EkaNyseBookSide(SIDE _side, EkaNyseBook* _parent) {
  parent = _parent;
