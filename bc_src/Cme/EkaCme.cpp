...
#include "EpmIlink3NewOrderTemplate.h"
...
/* ----------------------------------------------------- */
EkaCme::EkaCme(EkaDev* _dev) : EkaExch(_dev) {
  EKA_LOG("Opening CME Exch");
}
/* ----------------------------------------------------- */
EkaParser* EkaCme::newParser(EkaStrat* _strat) {
  EkaCmeParser* p = new EkaCmeParser(this, _strat);
  if (p == NULL) on_error("new EkaCmeParser == NULL");
  p->strat = _strat;
  return (EkaParser*)p;
}
...
EkaBook*  EkaCme::newBook(uint8_t coreId,EkaStrat* _strat, EkaProd* _prod) {
  return (EkaBook*) (new EkaCmeBook(dev,_strat,coreId,_prod));
}
...
inline void getDepthData(EkaBook* b, SIDE side, EkaCme::CmeDepthParams* dst, uint depth) {
  uint32_t accSize = 0;
...
int EkaCme::onTobChange(MdOut* mdOut, EkaBook* b, SIDE side) {
  CmeHwBookParams tobParams = {};
  ...
  //    ((EkaCmeBook*)b)->printBook();
    ((EkaCmeBook*)b)->checkIntegrity();
    if (side == BID) ((EkaCmeBook*)b)->bid->printSide();
    if (side == ASK) ((EkaCmeBook*)b)->ask->printSide();
    //  }
#else
    //  eka_side_type_t hwSide = side == BID ? eka_side_type_t::BUY : eka_side_type_t::SELL;
  if (side == BID)
    ekaWriteTob(mdOut->dev,&tobParams,sizeof(CmeHwBookParams),(eka_product_t)b->getProdId(),eka_side_type_t::BUY);
  if (side == ASK)
    ekaWriteTob(mdOut->dev,&tobParams,sizeof(CmeHwBookParams),(eka_product_t)b->getProdId(),eka_side_type_t::SELL);
...
int EkaCme::ekaDownloadProductParams(eka_product_t product_id) {
  ...
  //  product_param.ei_seq_num      = prod->ei_seq_num; not in eobi release
...
int EkaCme::jumpParamsApi2Internal(const jump_params_t* apiParams, volatile void* _dst) {
...
int EkaCme::rjumpParamsApi2Internal(const reference_jump_params_t* apiParams, volatile void* _dst) {
...
int EkaCme::sendDate2Hw(EkaDev* dev) {
  struct timespec t;
  clock_gettime(CLOCK_REALTIME, &t); 
  uint64_t current_time_ns = ((uint64_t)(t.tv_sec) * (uint64_t)1000000000 + (uint64_t)(t.tv_nsec)); //UTC time
  //  current_time_ns += static_cast<uint64_t>(6*60*60) * static_cast<uint64_t>(1e9); //+6h UTC time

  eka_write(dev,0xf0300+0*8,be64toh(current_time_ns)); //data, last write commits the change
...
EpmTemplate* EkaCme::addCancelMsgTemplate(uint templateId) {
  return new EpmIlink3CancelOrderTemplate(templateId,"EpmIlink3CancelOrderTemplate");
}
/* ----------------------------------------------------- */
EpmTemplate* EkaCme::addNewOrderMsgTemplate(uint templateId) {
  return new EpmIlink3NewOrderTemplate(templateId,"EpmIlink3NewOrderTemplate");
}
/* ----------------------------------------------------- */

int EkaCme::createCancelMsg(uint8_t* dst, void* params) {
...
int EkaCme::createNewOrder(uint8_t* dst, void* params) {
...
int EkaCme::updateCancelMsg(uint prodIdx, uint8_t* msg, void* params) {
...
int EkaCme::updateNewOrder(uint prodIdx, uint8_t* msg, void* params) {
