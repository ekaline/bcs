...
int createBooks(EkaStrat *strat, uint8_t coreId) {
  ...
  for (uint i = 0; i < strat->prodNum; i++) {
    EKA_LOG("Trying prod %u,prodNum = %u", i,
            strat->prodNum);
    fflush(stderr);
    if (strat->prod[i] == NULL)
      on_error("strat->prod[%u] == NULL", i);

    if (!strat->prod[i]->is_book)
      continue;
    EKA_LOG("Prod %u is book, prodNum = %u", i,
            strat->prodNum);
    fflush(stderr);

    if (coreId != strat->prod[i]->mdCoreId) {
      EKA_LOG(
          "skipping product %s (mdCoreId=%u) for core %u",
          EKA_RESOLVE_PROD(strat->hwFeedVer,
                           strat->prod[i]->product_id),
          strat->prod[i]->mdCoreId, coreId);
      continue;
    }
    if (strcmp(EKA_RESOLVE_PROD(strat->hwFeedVer,
                                strat->prod[i]->product_id),
               "---") == 0)
      on_error("Unsupported product id %u",
               (uint)strat->prod[i]->product_id);
...
int subscribeOnMc(EkaStrat *strat, uint8_t coreId) {
  ...
  for (uint i = 0; i < strat->udpSessions; i++) {
    ...
    if (strat->udpSess[i] == NULL)
      on_error("strat->udpSess[%u] == NULL", i);

    if (strat->dev->core[coreId] == NULL)
      on_error("strat->dev->core[%u] == NULL", coreId);

    if (coreId != strat->udpSess[i]->coreId)
      continue;
    ...
    EKA_LOG("Subscribing Udp sess %u on %s:<ALL_PORTS>", i,
            EKA_IP2STR(mcastIp));
    if (strat->udpChannel[coreId] == NULL)
      strat->udpChannel[coreId] =
          new EkaUdpChannel(strat->dev, coreId);

    if (strat->udpChannel[coreId] == NULL)
      on_error("strat->udpChannel[%u] == NULL", coreId);

    strat->udpChannel[coreId]->igmp_mc_join(srcIp, mcastIp,
                                            port, vlanTag);
  }
  return 0;
}
...
static bool productsBelongTo(EkaStrat *strat,
                             uint8_t coreId) {
  for (uint i = 0; i < strat->prodNum; i++) {
    if (strat->prod[i] == NULL)
      on_error("strat->prod[%u] == NULL", i);
...
/* ----------------------------------------------------- */
int ekaWriteTob(EkaDev *dev, void *params, uint paramsSize,
...
void bookLoopThread_f(EkaStrat *strat) {
  ...
  setThreadAffinityName(dev, pthread_self(),
                        "EkalineBookLoop",
  ...
  while (strat->threadActive) {
    for (uint coreId = 0; coreId < EkaDev::CONF::MAX_CORES;
         coreId++) {
      if (strat->udpChannel[coreId] == NULL)
        continue;
      if (!strat->udpChannel[coreId]->has_data())
        continue;
      uint8_t *pkt = NULL;
      if ((pkt = (uint8_t *)strat->udpChannel[coreId]
                     ->get()) == NULL)
        on_error("p == NULL");

      EkaIpHdr *ipH = (EkaIpHdr *)(pkt - 8 - 20);
      EkaUdpHdr *udpH = (EkaUdpHdr *)(pkt - 8);
      EkaUdpSess *udpSess = strat->findUdpSess(
          ipH->dest, be16toh((udpH->dest)));
      if (udpSess == NULL) {
      ...
      uint payloadSize =
          strat->udpChannel[coreId]->getPayloadLen();
#ifdef _EKA_PARSER_PRINT_ALL_
      //    EKA_LOG("Pkt: %u byte ",payloadSize);
#endif
      pkt += strat->parser->processPkt(
          &mdOut, ProcessAction::UpdateBookOnly, pkt,
          payloadSize);
      if (!udpSess->isCorrectSeq(
              mdOut.pktSeq, mdOut.msgNum)) { /* on_error("Sequence
                                                Gap"); */
      }
...
/*   if (dev->core[coreId] == NULL) on_error("dev->core[%u]
 * == NULL",coreId); */
/*   if (dev->core[coreId]->udpChannel == NULL)
 * on_error("dev->core[%u]->udpChannel == NULL",coreId); */
...
/*     if (! dev->core[coreId]->udpChannel->has_data())
 * continue; */
/*     uint8_t* pkt = NULL; */
/*     if ((pkt =
 * (uint8_t*)dev->core[coreId]->udpChannel->get()) == NULL)
 * on_error ("p == NULL");  */

/*     EkaIpHdr*   ipH     = (EkaIpHdr*) (pkt - 8 - 20); */
/*     EkaUdpHdr*  udpH    = (EkaUdpHdr*)(pkt - 8); */
/*     EkaUdpSess* udpSess =
 * dev->core[coreId]->findUdpSess(ipH->dest,be16toh((udpH->dest)));
 */
/*     if (udpSess == NULL) { */
/* #ifdef _EKA_PARSER_PRINT_ALL_ */
/*       EKA_LOG("%s:%u packet does not belog to our UDP
 * sessions",EKA_IP2STR((ipH->dest)),be16toh(udpH->dest));
 */
/* #endif */
...
/*     } */
/*     uint payloadSize =
 * dev->core[coreId]->udpChannel->getPayloadLen(); */
/* #ifdef _EKA_PARSER_PRINT_ALL_ */
/*     //    EKA_LOG("Pkt: %u byte ",payloadSize); */
/* #endif */
/*     pkt +=
 * dev->exch->parser->processPkt(&mdOut,ProcessAction::UpdateBookOnly,pkt,payloadSize);
 */
/*     if (!
 * udpSess->isCorrectSeq(mdOut.pktSeq,mdOut.msgNum)) { /\*
...
void runBook(EkaStrat *strat) {
  ...
  for (uint8_t coreId = 0; coreId < EkaDev::CONF::MAX_CORES;
       coreId++) {
