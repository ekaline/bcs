...
class lock_guard {
  private:
    ...
    lock_guard(const lock_guard<T> &);                  // No copy construction of lock_guard.
    lock_guard<T> & operator=(const lock_guard<T> &);   // No assignment of lock_guard.
...
class thread {
  public:
    ...
    /// (i.e. non-joinable).
    thread() : mHandle(0), mDataMutex(), mNotAThread(true)
#if defined(_TTHREAD_WIN32_)
    , mWin32ThreadID(0)
#endif
    {}
