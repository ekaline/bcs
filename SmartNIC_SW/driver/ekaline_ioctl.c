 ...
 * smartnic_ioctl(struct file *f, unsigned int op, unsigned
 * long data) at file main.c */
...
uint64_t ekaSnDriverVerNum;

if (copy_from_user(&eka_ioctl, (void *)data,
                   sizeof(eka_ioctl_t)))
  return -EFAULT;
PRINTK("EKALINE DEBUG: EKA IOCTL cmd=%u\n", eka_ioctl.cmd);

ekaSnDriverVerNum = EKA_VER_NUM;

switch (eka_ioctl.cmd) {
case EKA_DUMP:
  /* if ((rc = copy_to_user((void __user *)
  ...
  /*   PRINTK("EKALINE DEBUG: SMARTNIC_EKALINE_DATA: DUMP
   * nif %d, session %d failed\n", */
  /* 	    eka_ioctl.nif_num, */
...
case EKA_SET:
  ...
  if (ekaIoctlNif > SC_NO_NETWORK_INTERFACES ||
      ekaIoctlSess > EKA_SESSIONS_PER_NIF) {
    PRINTK("EKALINE ERROR: SMARTNIC_EKALINE_DATA: Wrong "
           "nif %llu or session %llu \n",
           ekaIoctlNif, ekaIoctlSess);
  }

  memcpy(&pDevExt->nif[ekaIoctlNif]
              .eka_private_data->eka_session[ekaIoctlSess],
         &eka_ioctl.eka_session, sizeof(eka_session_t));

  PRINTK("EKALINE DEBUG: EKA_SET nif %llu, session %llu, "
         "active=%u\n",
         ekaIoctlNif, ekaIoctlSess,
         eka_ioctl.eka_session.active);
  break;
case EKA_VERSION:
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  if (strlen(EKA_SN_DRIVER_BUILD_TIME) >
      EKA_BUILD_TIME_STRING_LEN - 1) {
    PRINTK(
        "EKALINE ERROR: strlen(EKA_SN_DRIVER_BUILD_TIME) "
        "%zu > EKA_BUILD_TIME_STRING_LEN - 1 (%d)\n",
        strlen(EKA_SN_DRIVER_BUILD_TIME),
        EKA_BUILD_TIME_STRING_LEN - 1);
    return -EFAULT;
  }
  if ((rc = copy_to_user(
           (void __user *)eka_ioctl.paramA,
           EKA_SN_DRIVER_BUILD_TIME,
           strlen(EKA_SN_DRIVER_BUILD_TIME)))) {
    PRINTK("EKALINE DEBUG: SMARTNIC_EKALINE_DATA: "
           "EKA_VERSION failed\n");
    return -EFAULT;
  }
  PRINTK("EKA_VERSION: %s\n", EKA_SN_DRIVER_BUILD_TIME);
  /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  if (strlen(EKA_SN_DRIVER_RELEASE_NOTE) >
      EKA_RELEASE_STRING_LEN - 1) {
    PRINTK(
        "EKALINE ERROR: strlen(EKA_SN_DRIVER_RELEASE_NOTE) "
        "%zu > EKA_RELEASE_STRING_LEN - 1 (%d)\n",
        strlen(EKA_SN_DRIVER_RELEASE_NOTE),
        EKA_RELEASE_STRING_LEN - 1);
    return -EFAULT;
  }
  if ((rc = copy_to_user(
           (void __user *)eka_ioctl.paramB,
  ...
  if ((rc = copy_to_user((void __user *)eka_ioctl.paramC,
                         &ekaSnDriverVerNum, 8))) {
    PRINTK("EKALINE DEBUG: SMARTNIC_EKALINE_DATA: "
           "EKA_VER_NUM (%llu) failed\n",
           ekaSnDriverVerNum);
    return -EFAULT;
  }
  PRINTK("EKA_VER_NUM: %llu\n", ekaSnDriverVerNum);
  break;
case EKA_DEBUG_ON:
  pDevExt->eka_debug = 1;
...
case EKA_IOREMAP_WC: {
  ...
  PRINTK("EKALINE DEBUG: EKA_IOREMAP_WC "
         "pDevExt->ekaline_wc_addr = 0x%016llx",
         pDevExt->ekaline_wc_addr);
} break;
case EKA_GET_IGMP_STATE: {
  struct sc_multicast_subscription_t *dstAddr =
      (struct sc_multicast_subscription_t *)
          eka_ioctl.paramA;
  PRINTK("%s:%u: EKALINE DEBUG: EKA_GET_IGMP_STATE: "
         "dstAddr %px == wcattr.paramA 0x%llx\n",
         __FILE__, __LINE__, dstAddr, eka_ioctl.paramA);
  if ((rc = copy_to_user(
           (void __user *)dstAddr,
           ...
           sizeof(
               pDevExt->igmpContext.global_subscriber)))) {
    PRINTK(
        "%s:%u: EKALINE DEBUG: EKA_GET_IGMP_STATE failed\n",
        __FILE__, __LINE__);
    return -EFAULT;
  }
  PRINTK(
      "EKALINE DEBUG: EKA_GET_IGMP_STATE copied %zu bytes",
