// oob_chan.c: in fbProcessOobChannel before:
// fbNicReceive(nif, pCurrentPacket); // Pass packet upwards
// into Linux network stack

const uint8_t *eka_frameHeader =
    sc_get_raw_payload(pCurrentPacket);
int eka_sizeOfEtherFrameHeader =
    GetSizeOfEthernetFrameHeader(
        (FbEtherFrameHeader *)eka_frameHeader);
struct iphdr *eka_ip =
    (struct iphdr
         *)&eka_frameHeader[eka_sizeOfEtherFrameHeader];

if (((FbEtherFrameHeader *)eka_frameHeader)->etherType ==
    0x0081)
  eka_ip = (struct iphdr *)(((char *)eka_ip) + 4); // VLAN
...
if (pDevExt->eka_debug)
  PRINTK(
      "EKALINE DEBUG: RX: eka_frameHeader=%p, eka_ip=%p,  "
      "etherType=0x%04x, eka_ip->protocol=0x%0x\n",
      eka_frameHeader, eka_ip,
...
if (pDevExt->eka_drop_all_rx_udp == 1 &&
  ...
  struct udphdr *udph =
      (struct udphdr *)((char *)eka_ip +
                        eka_ip->ihl * sizeof(uint32_t));
  if (pDevExt->eka_debug)
    PRINTK("EKALINE DEBUG: RX: DROPPING: udph=%p,  "
           "%d.%d.%d.%d -> %d.%d.%d.%d : %hu->%hu\n",
           udph, eka_saddr[0], eka_saddr[1], eka_saddr[2],
           eka_saddr[3], eka_daddr[0], eka_daddr[1],
           eka_daddr[2], eka_daddr[3], ntohs(udph->source),
           ntohs(udph->dest));
...
if (eka_ip->protocol == 0x6) { // TCP
  ...
  for (eks = 0; eks < EKA_SESSIONS_PER_NIF; eks++) {
    if (nif->eka_private_data->eka_session[eks].active == 0)
      continue;
    if (nif->eka_private_data->eka_session[eks].ip_saddr !=
        eka_ip->daddr)
      continue;
    if (nif->eka_private_data->eka_session[eks].ip_daddr !=
        eka_ip->saddr)
      continue;
    if (nif->eka_private_data->eka_session[eks].tcp_sport !=
        ntohs(tcph->dest))
      continue;
    if (nif->eka_private_data->eka_session[eks].tcp_dport !=
        ntohs(tcph->source))
      continue;
