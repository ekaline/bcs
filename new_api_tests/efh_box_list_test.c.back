...
int main(int argc, char *argv[]) {
  ...
  const EfhInitCtx efhInitCtx = {
    .ekaProps = &ekaProps,
    .numOfGroups = std::size(boxGroups),
    .coreId = 0,
    .recvSoftwareMd = true,
#ifdef EKA_TEST_IGNORE_DEFINITIONS
    .subscribe_all = true,
...
/* ------------------------------------------------- */
  memcpy(&runGrCtx[i], &runCtx, sizeof(runCtx));

  runGrCtx[i].numGroups = 4;
  runGrCtx[i].groups    = groups[i];

  efh_run_thread[i] = std::thread(efhRunGroups,pEfhCtx, &runGrCtx[i]);
  efh_run_thread[i].detach();
  i++;
/* ------------------------------------------------- */
  memcpy(&runGrCtx[i], &runCtx, sizeof(runCtx));

  runGrCtx[i].numGroups = 3;
  runGrCtx[i].groups    = groups[i];
