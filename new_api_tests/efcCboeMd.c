...
static int getAttr(int argc, char *argv[],		    
  ...
  int opt; 
  while((opt = getopt(argc, argv, ":s:fdh")) != -1) {  
    switch(opt) {  
    case 's':  
...
int main(int argc, char *argv[]) {
  ...
  getAttr(argc,argv,&runEfh,&fatalDebug,secIdFileName);
  ...
  auto securityList = new uint64_t[MaxSecurities];
  ...
  for (auto i = 0; i < subscribedNum; i++) {
    ...
    SecCtx secCtx = {
	.bidMinPrice       = static_cast<decltype(secCtx.bidMinPrice)>(0xFFFF),
	.askMaxPrice       = static_cast<decltype(secCtx.askMaxPrice)>(1),  
	.size              = 1,
