...
void fastpath_thread_f(EkaDev* dev, ExcConnHandle sess_id,uint p2p_delay) {
  ...
  char tx_buf[BUF_SIZE] = {0x11, 0x22};
  ...
  if (sent != (int)pkt_size) on_error ("sent (%d) != pkt_size (%u)",sent,pkt_size);
  ...
  while(1) usleep(0);
  if (rcvd != (int)pkt_size) on_error ("rcvd (%d) != pkt_size (%u)",rcvd,pkt_size);
