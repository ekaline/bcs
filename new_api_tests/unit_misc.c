...
int main(int argc, char *argv[]) {
  ...
  for (uint64_t i = 0; i < iterations; i ++) {
    ...
    if (SC_ERR_SUCCESS != SC_ReadUserLogicRegister(dev_id, ADDR2READ, &rtv[i])) on_error("SC_Read returned smartnic error code : %d",SC_GetLastErrorCode());
