...
int credAcquire(EkaCredentialType credType, EkaGroup group,
								const char *user, const struct timespec *leaseTime,
...
int main(int argc, char *argv[]) {
    ...
    getAttr(argc,argv,&serverIp,&serverTcpPort,
    ...
    for (auto i = 0; i < numTcpSess; i++) {
			...
			conn[i] = ekaBcTcpConnect(dev,coreId,inet_addr(serverIp.c_str()),serverTcpBasePort + i);
			if (conn[i] < 0) on_error("failed to open sock %d",i);
    ...
    const EkaBcTriggerParams efcEpmStrategyParams = {
      .triggerParams = triggerParam,
      .numTriggers   = std::size(triggerParam),
    };
    rc = ekaBcCmeFcMdInit(dev, &efcEpmStrategyParams);
    ...
    auto cmeHwCancelIdx = ekaBcAllocateCmeFireAction(dev);
    ...
    rc = ekaBcSetAction(dev,cmeHwCancelIdx,&ekaBcActionParams);
    if (rc != EKA_OPRESULT__OK)
    ...
    const EfcBCCmeFastCancelParams params = {
    ...
    // ==============================================
    ekaBcCmeFcAlgoInit(dev,&params);
    // ==============================================
    efcBcEnableController(dev, -1);
    // ==============================================
    ...
    for (auto i = 0; i < TotalInjects; i++) {
      if (rand() % 3) {
	efcBcEnableController(dev, 1, armVer++); //arm and promote TBD
	ExpectedFires++;
      }
      else {
	efcBcEnableController(dev, 1, armVer-1); //should be no arm
      }
