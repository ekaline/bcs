...
static int getAttr(int argc, char *argv[],
		   ...
		   bool* fatalDebug, bool* dontExit,bool *reportOnly) {
	int opt; 
...
int main(int argc, char *argv[]) {
	...
	std::string clientIp        = "100.0.0.110"; 
	std::string triggerIp       = "233.54.12.111";
	...
	getAttr(argc,argv,&serverIp,&serverTcpPort,
		...
		&numTcpSess,&runEfh,&fatalDebug,&dontExit,&reportOnly);
	...
	ExcUdpTxConnHandle udpConn =
		excUdpConnect(dev,coreId,
	...
	auto itchFsActionIdx = efcAllocateNewAction(dev,
																							EpmActionType::ItchHwFastSweep);

	rc = efcSetAction(dev,itchFsActionIdx,&genericActionParams);
	if (rc != EKA_OPRESULT__OK)
		on_error("efcSetAction returned %d",(int)rc);

	rc = efcSetActionPayload(dev,itchFsActionIdx,
													 ItchTestFastSweepMsg,
													 18/* strlen(CmeTestFastCancelMsg) */);
	if (rc != EKA_OPRESULT__OK) 
