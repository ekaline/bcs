...
void excSendThr_f(EkaDev *pEkaDev, ExcConnHandle hCon,
                  uint thrId, uint p2p_delay) {
  uint8_t coreId = excGetCoreId(hCon);
  uint8_t sessId = excGetSessionId(hCon);
  TEST_LOG("Launching excSendThr for coreId %u, sessId %u, "
           "p2p_delay=%u",
  ...
  while (g_keepWork) {
    ...
    if (bufCnt % STATISTICS_PERIOD == 0)
      TEST_LOG("CoreId %u, SessId %u, bufCnt: %ju", coreId,
               sessId, bufCnt);
    if (g_bufVerified[thrId]) {
      ...
      while (g_keepWork && sentBytes < BufSize) {
        /* static const size_t PktSize = 1360; */ // large
                                                  // pkt
        /* size_t currPktSize = std::min(BufSize -
         * sentBytes, */
        /* 			      1 + rand() % PktSize);
         */
        size_t currPktSize =
            std::min(BufSize - sentBytes, (uint64_t)708);
        ...
        case 0:
          if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            g_keepWork = false;
            EKA_ERROR("Unexpected errno=%d (\'%s\')", errno,
                      strerror(errno));
          }
          break;
        default:
...
/* --------------------------------------------- */
void excRecvThr_f(EkaDev *pEkaDev, ExcConnHandle hCon,
                  uint thrId) {
  uint8_t coreId = excGetCoreId(hCon);
  uint8_t sessId = excGetSessionId(hCon);
  ...
  while (g_keepWork) {
    int rx =
        excRecv(pEkaDev, hCon, rxP, BufSize - rxCnt, 0);
    if (rx <= 0) {
      if (errno != EAGAIN)
    ...
    if (rxCnt == BufSize) {
      if (memcmp((const void *)g_bufPtr[thrId], rxBuf,
                 BufSize) != 0) {
...
void printUsage(char *cmd) {
  printf(
      "USAGE: %s -s <Connection Server IP> -p <Connection "
      "Server TcpPort> -c <Connection Client IP> -t "
      "<testSessions> -r <testCores> -d <p2p_delay> \n",
      cmd);
}
...
static int getAttr(int argc, char *argv[],
                   ...
                   std::string *clientIp,
                   uint *testSessions, uint *testCores,
                   uint *p2p_delay) {
  int opt;
  while ((opt = getopt(argc, argv, ":c:s:p:d:t:r:h")) !=
         -1) {
    switch (opt) {
    case 's':
...
void tcpChild(EkaDev *dev, int sock, uint port) {
  ...
  //  printf ("Running TCP Server for sock=%d, port =
  //  %u\n",sock,port);
  do {
    char line[1536] = {};
...
void tcpServer(EkaDev *dev, std::string ip, uint16_t port) {
  ...
  while (g_keepWork) {
    ...
    childSock = accept(sd, (struct sockaddr *)&addr,
                       (socklen_t *)&addr_size);
    TEST_LOG("Connected from: %s:%d -- sock=%d\n",
             inet_ntoa(addr.sin_addr), ntohs(addr.sin_port),
             childSock);
    std::thread child(tcpChild, dev, childSock,
                      be16toh(addr.sin_port));
    child.detach();
  }
  TEST_LOG(" -- closing");
...
int main(int argc, char *argv[]) {
  ...
  testConnection conn[MaxTestCores] = {
      {std::string("100.0.0.110"), std::string("10.0.0.10"),
       55555},
      {std::string("200.0.0.111"), std::string("10.0.0.11"),
       22223}};

  getAttr(argc, argv, &conn[0].dstIp, &conn[0].dstTcpPort,
          &conn[0].srcIp, &testSessions, &testCores,
          &p2p_delay);
  //-----------------------------------------
  printf("Running with %u cores, %u sessions, %uus "
         "pkt-to-pkt delay\n",
  ...
  for (uint8_t c = 0; c < testCores; c++) {
    for (uint8_t s = 0; s < testSessions; s++) {
      ...
      if (sock[c][s] < 0)
        on_error("failed to open sock[%u][%u]", c, s);

      struct sockaddr_in dst = {};
      dst.sin_addr.s_addr =
          inet_addr(conn[c].dstIp.c_str());
      dst.sin_family = AF_INET;
      dst.sin_port = htons(conn[c].dstTcpPort);

      TEST_LOG(
          "Trying to connect connect sock[%u][%u] on %s:%u",
          c, s, inet_ntoa(dst.sin_addr),
          conn[c].dstTcpPort);
      fflush(stderr);
      if ((hCon[c][s] = excConnect(
               pEkaDev, sock[c][s], (sockaddr *)&dst,
               sizeof(sockaddr_in))) < 0)
        on_error(
            "failed to connect sock[%u][%u] on port %u", c,
            s, conn[c].dstTcpPort);
  ...
  for (uint8_t c = 0; g_keepWork && c < testCores; c++) {
    for (uint8_t s = 0; g_keepWork && s < testSessions;
         s++) {
      uint thrId = c * testSessions + s;
      g_sessRcvThreadReady[thrId] = false;
      excRecvThr[c][s] = std::thread(excRecvThr_f, pEkaDev,
                                     hCon[c][s], thrId);
      while (g_keepWork && !g_sessRcvThreadReady[thrId])
        std::this_thread::yield();

      excSendThr[c][s] =
          std::thread(excSendThr_f, pEkaDev, hCon[c][s],
                      thrId, p2p_delay);
    }
  }
  //------------------------------------------------
