...
EkaOpResult EkaFhCme::initGroups(EfhCtx*          pEfhCtx, 
  ...
  for (uint8_t i = 0; i < runGr->numGr; i++) {
    ...
    auto gr = dynamic_cast<EkaFhCmeGr*>(b_gr[localId]);
    if (! gr)
			on_error ("b_gr[%u] == NULL",localId);
    ...
    EKA_DEBUG("%s:%u: joined Incr Feed %s:%u, "
							"Recovery Feed %s:%u, for %u securities",
...
EkaOpResult EkaFhCme::runGroups( EfhCtx* pEfhCtx,
																 const EfhRunCtx* pEfhRunCtx,
																 uint8_t runGrId ) {
  ...
  while (runGr->thread_active) {
    ...
    if (! pkt) continue;
      ...
      pkt = static_cast<uint8_t *>(std::memcpy(inlinePktBuf,
																							 pkt, pktSize));
    ...
    if (sequence != gr->expected_sequence) { // GAP
      EKA_LOG("%s:%u Gap at FH_LAB NORMAL:  "
							"gr->expected_sequence=%ju, sequence=%ju, gap=%jd",
							EKA_EXCH_DECODE(exch),gr_id,
    ...
    case EkaFhGroup::GrpState::NORMAL : {
      ...
      if (sequence > gr->expected_sequence) { // GAP
				EKA_LOG("%s:%u Gap at NORMAL:  "
								"gr->expected_sequence=%ju, sequence=%ju, gap=%ju",
								EKA_EXCH_DECODE(exch),gr_id,
								gr->expected_sequence,sequence,
								sequence - gr->expected_sequence);
				...
				if (gr->processPkt(pEfhRunCtx,pkt,pktSize,EkaFhMode::MCAST))
					break;
				gr->expected_sequence = sequence + 1;
      }
    }
...
EkaOpResult EkaFhCme::getDefinitions (EfhCtx* pEfhCtx,
																			const EfhRunCtx* pEfhRunCtx,
																			const EkaGroup* group) {
  auto gr = dynamic_cast<EkaFhCmeGr*>(b_gr[group->localId]);
  if (gr == NULL)
		on_error("gr[%u] == NULL",(uint8_t)group->localId);
