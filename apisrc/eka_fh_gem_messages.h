...
//#define EFH_GEMX_STRIKE_PRICE_SCALE 100000
#define EFH_GEMX_STRIKE_PRICE_SCALE 10000

struct tqf_generic_header { // Dummy
  char		type;             // 1
  char	        time_nano[6];     // 6
} __attribute__((packed));

struct tqf_best_bid_or_ask_update_short {
  char		type; // 'b' or 'a'
  char	        time_nano[6];
  uint32_t	option_id;
...
struct tqf_best_bid_or_ask_update_long {
  char		type; // 'B' or 'A'
  char	        time_nano[6];
  uint32_t	option_id;
  char          quote_condition;
...
struct tqf_security_open_close {
  char     message_type; // 'O'
  char     time_nano[6];
  uint32_t option_id;
...
struct tqf_best_bid_and_ask_update_short {
  char     message_type; // 'q'
  char     time_nano[6];
  uint32_t option_id;
...
struct tqf_best_bid_and_ask_update_long {
  char     message_type; // 'Q'
  char     time_nano[6];
  uint32_t option_id;
  char     quote_condition; // <space> = regular quote/autox eligible “X” = not autox eligible
