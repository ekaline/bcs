...
public:
  ...
  EkaOpResult runGroups( EfhCtx* pEfhCtx, const EfhRunCtx* pEfhRunCtx, uint8_t runGrId );
  EkaOpResult initGroups(EfhCtx* pEfhCtx, const EfhRunCtx* pEfhRunCtx, FhRunGr* runGr);
  uint8_t*    getUdpPkt(FhRunGr* runGr, uint* msgInPkt, uint64_t* sequence, uint8_t* gr_id, uint16_t* streamId, uint8_t* pktType);
  bool        processUdpPkt(const EfhRunCtx* pEfhRunCtx,FhXdpGr* gr, uint streamIdx, const uint8_t* pktPtr, uint msgInPkt, uint64_t seq);
