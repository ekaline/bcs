...
EkaOpResult EkaFhPlr::runGroups( EfhCtx* pEfhCtx,
  ...
  while (runGr->thread_active && ! runGr->stoppedByExchange) {
    ...
    auto gr {dynamic_cast<EkaFhPlrGr*>(b_gr[gr_id])};
    if (!gr)
    ...
    if (gr->state == EkaFhGroup::GrpState::NORMAL && 
      ...
      EKA_WARN("%s:%u: TEST GAP INJECTED: "
							 ...
							 "pkt sequence %u with unknown number "
							 "of messages dropped",
    ...
    case EkaFhGroup::GrpState::INIT : {
      if (gr->skipSnapshot()) {
				EKA_LOG("%s:%u Skipping Snapshot for pure Trade "
								"group: sequence=%u",
      ...
      gr->closeSnapshotGap(pEfhCtx,pEfhRunCtx, (uint64_t)0,
													 (uint64_t)0);
    }
      break;
      //---------------------------------------
    case EkaFhGroup::GrpState::NORMAL : {     
      ...
      if (sequence < gr->expected_sequence) {
				EKA_WARN("%s:%u BACK-IN-TIME WARNING: "
								 "sequence %u < expected_sequence %ju",
								 EKA_EXCH_DECODE(exch),gr_id,
      ...
      if (sequence > gr->expected_sequence) { // GAP
				EKA_LOG("%s:%u Gap at NORMAL:  "
								"gr->expected_sequence=%ju, sequence=%u, msgInPkt=%u",
								EKA_EXCH_DECODE(exch),gr_id,
								gr->expected_sequence,sequence,msgInPkt);
...
EkaOpResult EkaFhPlr::getDefinitions (EfhCtx* pEfhCtx,
																			const EfhRunCtx* pEfhRunCtx,
																			const EkaGroup* group) {
  auto gr {dynamic_cast<EkaFhPlrGr*>(b_gr[(uint8_t)group->localId])};
  if (! gr)
    on_error("%s:%u Group does not exist (gr == NULL)",
						 EKA_EXCH_DECODE(exch),gr->id);
  ...
  if (gr->productMask == PM_VanillaTrades) {
    ...
    EKA_DEBUG("%s:%u: skipping definitions for trade group",
							EKA_EXCH_DECODE(exch),gr->id);
    return EKA_OPRESULT__OK;
  }
