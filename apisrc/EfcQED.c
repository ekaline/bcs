...
EkaOpResult efcQEDInit(EkaDev *dev,
  ...
  volatile EfcQEDStrategyConf conf = {
    .product[0].enable       = params->product[0].enable;
    .product[0].minNumLevel  = params->product[0].min_num_level;
    .product[0].dsID         = params->product[0].ds_id;
    .product[0].token        = params->product[0].token;
    .product[0].fireActionId = params->product[0].fireActionId;
    .product[0].strategyId   = (uint8_t)EFC_STRATEGY;
  };
  ...
  for (auto i = 0; i < 4; i++) {
    EKA_LOG("Configuring QED FPGA: product=%d, enable=%d, min_num_level=%d,ds_id=0x%x,token=0x%jx,fireActionId=%d,strategyId=%d",
	    ...
	    conf->product[i].min_num_level,
	    conf->product[i].ds_id,
