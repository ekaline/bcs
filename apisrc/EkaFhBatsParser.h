...
#define EKA_PRINT_BATS_SYMBOL(x)                           \
  ((std::string((x), 6)).c_str())
#define EKA_PRINT_BATS_SYMBOL_EXP(x)                       \
  ((std::string((x), 8)).c_str())
...
#define EKA_BATS_PITCH_MSG_DECODE(x)                       \
  ...
  : x == MsgId::ORDER_EXECUTED_AT_PRICE_SIZE               \
  ...
  : x == MsgId::OPTIONS_AUCTION_UPDATE                     \
      ? "OPTIONS_AUCTION_UPDATE"                           \
  : x == MsgId::AUCTION_SUMMARY ? "AUCTION_SUMMARY"        \
  : x == MsgId::AUCTION_NOTIFICATION                       \
  ...
  : x == MsgId::RETAIL_PRICE_IMPROVEMENT                   \
      ? "RETAIL_PRICE_IMPROVEMENT"                         \
  : x == MsgId::SOQ_STRIKE_RANGE_UPDATE                    \
      ? "SOQ_STRIKE_RANGE_UPDATE"                          \
  : x == MsgId::CONSTITUENT_SYMBOL_MAPPING                 \
      ? "CONSTITUENT_SYMBOL_MAPPING"                       \
  : x == MsgId::UNIT_CLEAR        ? "UNIT_CLEAR"           \
  : x == MsgId::TRANSACTION_BEGIN ? "TRANSACTION_BEGIN"    \
  : x == MsgId::TRANSACTION_END   ? "TRANSACTION_END"      \
  : x == MsgId::COMPLEX_INSTRUMENT_DEFINITION_EXPANDED     \
      ? "COMPLEX_INSTRUMENT_DEFINITION_EXPANDED"           \
      : "NON_INTERESTING"

inline bool operator==(const MsgId lhs,
                       const uint8_t rhs) noexcept {
  return ((uint8_t)lhs) == rhs;
}
inline bool operator==(const uint8_t lhs,
                       const MsgId rhs) noexcept {
...
/* ------------------------------------------------ */
static inline EfhTradeStatus
tradeAction(EfhTradeStatus prevTradeAction,
            char msgTradeStatus) {
  ...
  default:
    on_error("Unexpected trade status \'%c\'",
             msgTradeStatus);
  }
  return prevTradeAction;
}
/* ------------------------------------------------ */
inline bool checkPriceLengh(uint64_t price,
                            int priceScale = 1) {
...
/* ------------------------------------------------ */
inline FhOrderType addFlagsCustomerIndicator2orderType(
    uint8_t flags, char customerIndicator) {
...
getVanillaAuctionType(char msgAuctionType, bool useAIM) {
  switch (msgAuctionType) {
  case 'B':
    return useAIM ? EfhAuctionType::kAIM
                  : EfhAuctionType::kBatsAuctionMechanism;
  case 'S':
    return EfhAuctionType::kSolicitation;
  case 'T':
    return EfhAuctionType::kStepUpMechanism;
  case 'A':
    return EfhAuctionType::kAllOrNone;
  default:
    return EfhAuctionType::kUnknown;
  }
}

/* ------------------------------------------------ */
inline EfhAuctionType
getComplexAuctionType(char msgAuctionType) {
  switch (msgAuctionType) {
  case 'C':
    return EfhAuctionType::kComplexOrderAuction;
  case 'S':
    return EfhAuctionType::kSolicitation;
  case 'B':
    return EfhAuctionType::kAIM;
  case 'O':
    return EfhAuctionType::kAllOrNone;
  default:
    return EfhAuctionType::kUnknown;
...
inline uint32_t normalize_bats_symbol_char(char c) {
  if (c >= '0' && c <= '9')
    return (uint32_t)(0 + (c - '0')); // 0..9
  if (c >= 'A' && c <= 'Z')
    return (uint32_t)(10 + (c - 'A')); // 10..35
  if (c >= 'a' && c <= 'z')
    return (uint32_t)(36 + (c - 'a')); // 36..61
  on_error("Unexpected symbol |%c|", c);
}
/* ------------------------------------------------ */

inline uint32_t bats_symbol2optionid(const char *s,
                                     uint symbol_size) {
  uint64_t compacted_id = 0;
  if (s[0] != '0')
    on_error("%c%c%c%c%c%c doesnt have \"0\" prefix", s[0],
             s[1], s[2], s[3], s[4], s[5]);
  for (uint i = 1; i < symbol_size; i++) {
    //    printf ("s[%i] = %c, normalize_bats_symbol_char =
    //    %u = 0x%x\n",i,s[i],
    //    normalize_bats_symbol_char(s[i]),
    //    normalize_bats_symbol_char(s[i]));
    compacted_id |=
        normalize_bats_symbol_char(s[i])
        << ((symbol_size - i - 1) * symbol_size);
  }
  if ((compacted_id & 0xffffffff00000000) != 0)
    on_error("%c%c%c%c%c%c encoding produced 0x%jx "
             "exceeding 32bit",
...
struct sequenced_unit_header { //
  uint16_t length; // 2 -- length of entire block of
                   // messages including this header
  uint8_t count;   // 1 -- number of messages to follow this
                   // header
  uint8_t unit; // 1 -- Unit to applies to messages included
                // in this header
  uint32_t sequence; // 4 -- Sequence of first message to
                     // follow this header
} __attribute__((packed));

#define EKA_BATS_MSG_CNT(x)                                \
  ((((sequenced_unit_header *)x)->count))
#define EKA_BATS_SEQUENCE(x)                               \
  ((((sequenced_unit_header *)x)->sequence))
#define EKA_BATS_UNIT(x)                                   \
  ((((sequenced_unit_header *)x)->unit))
...
struct GenericHeader { // presents at all messages excepting
                       // "Symbol Mapping"
  uint8_t length;      // 1
  uint8_t type;        // 1
  uint32_t time;       // 4
...
struct add_order_expanded { // 0x2F
  ...
  char customer_indicator; // 1 'N' - Non-Customer, 'C' -
                           // Customer
  char client_id[4];       // 4
} __attribute__((packed));
...
struct order_executed_at_price_size { // 0x24
  ...
  uint32_t executed_size; // 4
  uint32_t
      remaining_size; // 4 -- IGNORED IN THE BOOK LOGIC!!!
  uint64_t execution_id; // 8
  uint64_t price;        // 8
  char trade_condition;  // 1
  /* ' ' (Space) - Normal Trade */
...
struct order_modify_long { // 0x27
   ...
   */
  /*   bit 1 - Maintain Priority: '0' - Reset Priority, '1'
   * - Maintain Priority */
  /*   bits 2-7 - Reserved */
} __attribute__((packed));
...
struct SymbolMapping {   // 0x2E
  ...
  char symbol_condition; // 1  'N' - Normal, 'C' - Closing
                         // Only
...
struct AuctionNotification { // 0xAD
  ...
  char symbol[6];
  uint64_t auctionId;
  char auctionType; // 'B' = Bats Auction Mechanism (BAM)
                    // (EDGX Only) or AIM (C1 Only) 'S' =
  ...
  uint32_t auctionEndOffset; // Nanosecond offset from last
                             // timestamp
  uint32_t clientId;
} __attribute__((packed));
...
struct OptionsAuctionUpdate { // 0xD1
  GenericHeader header;
  char symbol[8];
  char auctionType; // 'G' = GTH Opening, 'O' = RTH Opening,
                    // 'H' = Halt Re-Opening, 'V' =
                    // Volatility Opening
...
struct OptionsAuctionUpdate_complex { // 0xD1
  ...
  int64_t
      compositeMarketOfferPrice; // Unused in complex, = 0
} __attribute__((packed));
...
getRfqCapacity(char customerIndicator) {
  ...
  default:
    on_error("Unexpected customerIndicator\'%c\'",
             customerIndicator);
  }
}
...
inline EfhSecurityType getComplexSecurityType(char type) {
  ...
  default:
    on_error("Unexpected LegSecurityType \'%c\'", type);
  }
}
...
struct ComplexInstrumentDefinitionExpanded_root {
  ...
  char ComplexInstrumentType
      [4]; // 4 character field; each field describes a
           // characteristic. Character 1: Complex Option
           // Type
...
struct GapResponse { // 0x04
  ...
  char status;       // 1
  /* 'A' - Accepted */
  /* 'O' - Out of range (ahead of sequence or too far
   * behind) */
  ...
  /* 'C' - Count request limit for one gap request is
   * exceeded */
...
struct SpinResponse { // 0x82 for snapshot
  ...
  uint8_t length;    // 1
  uint8_t type;      // 1
  uint32_t sequence; // 4 Sequence number from
                     // spin_image_available or '0' for
                     // Definitions
  uint32_t count; // 4 Number of "Add Order" or Definitions
                  // messages which will be contained in
                  // this spin
  char status;    // 1
  /* 'A' - Accepted */
  /* 'O' - Out of range  */
...
struct SpinRequest { // 0x84 for instrument_definition
  ...
  uint8_t type;   // 1
  uint32_t
      sequence; // 4 Must be '0' for instrument_definition,
                // from spin_image_available for snapshot
} __attribute__((packed));
...
inline uint64_t expSymbol2secId(const char (&s)[8]) {
  if (s[6] != ' ' || s[7] != ' ')
    on_error("ADD_ORDER_EXPANDED message with "
             "\'%c%c%c%c%c%c%c%c\' symbol (longer than 6 "
             "chars) not supported",
