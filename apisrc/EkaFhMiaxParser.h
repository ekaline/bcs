...
namespace Tom {
  ...
  inline bool operator==(const TOM_MSG lhs, const uint8_t rhs) noexcept { return ((uint8_t)lhs) == rhs; }
  inline bool operator==(const uint8_t lhs, const TOM_MSG rhs) noexcept { return lhs == (uint8_t)rhs;   }
  inline bool operator!=(const TOM_MSG lhs, const uint8_t rhs) noexcept { return ((uint8_t)lhs) != rhs; }
  inline bool operator!=(const uint8_t lhs, const TOM_MSG rhs) noexcept { return lhs != (uint8_t)rhs;   }
...
#define EKA_MIAX_TRADE_STAT(x)   (x == 'T' ? EfhTradeStatus::kNormal : EfhTradeStatus::kHalted)
...
#define EKA_GET_MACH_SEQ(x)      ((((Tom::EkaMiaxMach*)(x))->sequence))
#define EKA_GET_MACH_LEN(x)      ((((Tom::EkaMiaxMach*)(x))->length))
#define EKA_GET_MACH_TYPE(x)     ((((Tom::EkaMiaxMach*)(x))->type))
#define EKA_GET_MACH_SESS(x)     (((Tom::EkaMiaxMach*)(x))->sess)
#define EKA_IS_MACH_HEARTBEAT(x) ((((Tom::EkaMiaxMach*)(x))->type == Tom::MiaxMachType::Heartbeat))
}
#endif
