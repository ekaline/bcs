...
/* ############################################### */
EkaHwHashTableLine::EkaHwHashTableLine(
    EkaDev *_dev, EfhFeedVer _hwFeedVer, int _id) {
  dev = _dev;
...
uint16_t EkaHwHashTableLine::getHash(uint64_t normSecId) {
  ...
  case EfhFeedVer::kCBOE:
    return (normSecId >> EFC_SUBSCR_TABLE_ROWS_BITS) &
           0xFFFF;

  case EfhFeedVer::kMIAX:
    return (normSecId >> 24) & 0xFF;
    //    return (((normSecId >> 24) & 0x3E) | (( normSecId
    //    >> 15) & 0x1)); // 6 bits
  ...
  for (uint32_t i = 0; i < validCnt; i++) {
    if (col[i].hash == hash) {
      if (col[i].secId != secId)
        on_error("Hash error! Hash line %d: Securities %jx "
                 "(%ju) and %jx (%ju) have same hash %04x",
                 id, secId, secId, col[i].secId,
                 col[i].secId, hash);
...
/* ############################################### */
int EkaHwHashTableLine::printLine(const char *msg) {
  char str[16000] = {};
  char *s = str;
  s += sprintf(s, "%s: line %d: validCnt = %u, sum=%u, ",
               msg, id, validCnt, sum);
  for (auto i = 0; i < validCnt; i++) {
    s += sprintf(s, "%jx:%x, ", col[i].secId, col[i].hash);
  }
  EKA_LOG("%s", str);
  ...
  EKA_LOG("%s: line %d: validCnt = %u, sum=%u, %jx:%x, "
          "%jx:%x, %jx:%x, %jx:%x",
          msg, id, validCnt, sum, col[0].secId, col[0].hash,
          col[1].secId, col[1].hash, col[2].secId,
          col[2].hash, col[3].secId, col[3].hash);
#endif
...
/*   uint8_t period_groups = (EFC_SUBSCR_TABLE_COLUMNS / 4)
 * + (EFC_SUBSCR_TABLE_COLUMNS % 4 == 0 ? 0 : 1); // */
/*   for (uint8_t gr_per = 0; gr_per < period_groups;
 * gr_per++) { // running 42/4 = 11 group */
/*     uint8_t curr0 = s_col   < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col  ].hash     ) & 0x3F : 0; */
/*     uint8_t next0 = s_col+1 < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col+1].hash << 6) & 0xC0 : 0; */
/*     dst[d_col] = curr0 | next0; */

/*     uint8_t curr1 = s_col+1 < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col+1].hash >> 2) & 0x0F : 0; */
/*     uint8_t next1 = s_col+2 < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col+2].hash << 4) & 0xF0 : 0; */
/*     dst[d_col+1] = curr1 | next1; */

/*     uint8_t curr2 = s_col+2 < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col+2].hash >> 4) & 0x03 : 0; */
/*     uint8_t next2 = s_col+3 < EFC_SUBSCR_TABLE_COLUMNS ?
 * (col[s_col+3].hash << 2) & 0xFC : 0; */
