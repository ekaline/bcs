...
// Coped from <lwip/sockets.h>, needed to get the LWIP
...
#define FIONREAD_LWIP                                      \
  LWIP__IOR('f', 127,                                      \
            unsigned long) /* get # bytes to read */

extern "C" {

int lwip_poll(struct pollfd *fds, int nfds, int timeout);
int lwip_getsockopt(int fd, int level, int optname,
                    void *optval, socklen_t *optlen);
int lwip_setsockopt(int fd, int level, int optname,
...
// Linux and lwIP number their socket options differently.
// In general the levels are the same, except for
// SOL_SOCKET. NOTE: you *cannot* include <lwip/socket.h>
// here or it will re#define all the important macros. If
// lwIP is ever upgraded, this mapping must be kept in sync
// by hand.
constexpr int
mapLinuxSocketOptionLevelToLWIP(int linuxLevel) {
  return linuxLevel == SOL_SOCKET ? 0xfff : linuxLevel;
}

// Linux and lwIP use very different numbering for option
// names. We only map support options that we care about.
constexpr int
mapLinuxSocketOptionNameToLWIP(int linuxLevel,
                               int linuxName) {
  switch (linuxLevel) {
  case SOL_SOCKET:
    switch (linuxName) {
...
// Linux and LWIP also map their poll(2) constants
// differently.
...
int linuxPollEventsToLWIP(int linuxEvents,
                          int *unsupported) {
  int lwipEvents = 0;
  ...
  for (int b = 0; linuxEvents; ++b) {
    ...
    if (linuxEvents & linuxFlag) {
      const auto lwipFlag =
          mapLinuxPollFlagToLWIP(linuxFlag);
      if (lwipFlag == 0)
        on_error("lwip does not map define poll flag %x",
                 linuxFlag);
      else if (!isLinuxPollFlagSupported(linuxFlag) &&
               unsupported)
...
constexpr int lwipPollEventsToLinux(int lwipEvents) {
  ...
  for (int b = 0; lwipEvents; ++b) {
    ...
    if (lwipEvents & lwipFlag) {
      const auto linuxFlag =
          mapLWIPPollFlagToLinux(lwipFlag);
      linuxEvents |= linuxFlag;
      lwipEvents &= ~lwipFlag;
    }
  }
...
/**
 * This is a utility function that will return the
 * ExcSessionId from the result of exc_connect.
 *
 * @param hConnection This is the value returned from
 * exc_connect()
 * @return This will return the ExcSessionId value that
 * corresponds to excSessionId.
 */
...
/**
 * This is a utility function that will return the coreId
 * from the result of exc_connect.
 *
 * @param hConnection This is the value returned from
 * exc_connect()
 * @return This will return the ExcCoreid that corresponds
 * to excSessionId.
 */
...
inline EkaCore *getEkaCore(EkaDev *dev, EkaCoreId coreId) {
  if (!checkDevice(dev))
    return nullptr;
           ...
           unsigned(coreId) >= std::size(dev->core)) {
    errno = EBADF;
    return nullptr;
  }
...
inline EkaTcpSess *getEkaTcpSess(EkaDev *dev,
                                 ExcConnHandle hConn) {
  if (EkaCore *const core =
          getEkaCore(dev, excGetCoreId(hConn))) {
    const auto sessionId = excGetSessionId(hConn);
    if (sessionId < 0 ||
        unsigned(sessionId) >= std::size(core->tcpSess)) {
      errno = EBADF;
      return nullptr;
    }
...
ExcSocketHandle excSocket(EkaDev *dev, EkaCoreId coreId,
  ...
  // int domain, int type, int protocol parameters are
  // ignored always used: socket(AF_INET, SOCK_STREAM, 0)
 ...
 *
 */
ExcConnHandle excConnect(EkaDev *dev,
                         ExcSocketHandle hSocket,
...
/**
 * This will be called when we need to reconnect to a socket
 * that has already been disconnected. Behavior is undefined
 * if the firing controller is set up to fire on this socket
 * when this is called.
 */
ExcConnHandle excReconnect(EkaDev *pEkaDev,
                           ExcConnHandle hConn) {
  // TBD
  return 0;
}
...
/**
 * $$NOTE$$ This is mutexed to handle single session at a
 * time.
 *
 * @param hConn
 * @param pBuffer
 * @param size
 * @param flag    This should either be a standard linux
 * flag, or the result of ekaGetCapsResult(
 * kEkaCapsSendWarmupFlag ).  If it's the latter, then the
 * packet shouldnt actually go on the wire, but the software
 *                path should be warmed up.
 * @return This will return the values that exhibit the same
 * behavior of linux's send fn.
 */
ssize_t excSend(EkaDev *dev, ExcConnHandle hConn,
                const void *pBuffer, size_t size,
                int flags) {
  if (EkaTcpSess *const s = getEkaTcpSess(dev, hConn)) {
    return s->sendPayload(s->sessId /* thrId */,
                          (void *)pBuffer, size,
                          linuxMsgFlagsToLWIP(flags));
  }
  return -1;
}
 ...
 * time.
 */
ssize_t excRecv(EkaDev *dev, ExcConnHandle hConn,
                void *pBuffer, size_t size, int flags) {
  if (EkaTcpSess *const s = getEkaTcpSess(dev, hConn))
...
int excPoll(EkaDev *dev, struct pollfd *fds, int nfds,
            int timeout) {
  if (!checkDevice(dev))
    return -1;
  else if (!nfds) {
    // lwip_poll asserts if nfds == 0, we just return 0 in
    // that case.
    return 0;
  }

  auto *const lwipPollFds =
      static_cast<pollfd *>(alloca(sizeof(pollfd) * nfds));
  ...
  for (int i = 0; i < nfds; ++i) {
    if (fds[i].fd < 0)
      ...
      lwipPollFds[i].events = linuxPollEventsToLWIP(
          fds[i].events, &unsupported);
    }
  }
  ...
  if (rc != -1) {
    for (int i = 0; i < nfds; ++i)
      fds[i].revents =
          lwipPollEventsToLinux(lwipPollFds[i].revents);
  }
  return rc;
}

int excGetSockOpt(EkaDev *dev, ExcSocketHandle hSock,
                  int level, int optname, void *optval,
                  socklen_t *optlen) {
  if (checkDevice(dev)) {
    optname = mapLinuxSocketOptionNameToLWIP(
        level, optname); // Must be first because
    level = mapLinuxSocketOptionLevelToLWIP(
        level); // we change level here
    return lwip_getsockopt(hSock, level, optname, optval,
                           optlen);
  }
  return -1;
}

int excSetSockOpt(EkaDev *dev, ExcSocketHandle hSock,
                  int level, int optname,
                  const void *optval, socklen_t optlen) {
  if (checkDevice(dev)) {
    optname =
        mapLinuxSocketOptionNameToLWIP(level, optname);
    level = mapLinuxSocketOptionLevelToLWIP(level);
    return lwip_setsockopt(hSock, level, optname, optval,
                           optlen);
  }
  return -1;
}
...
int excGetBlocking(EkaDev *dev, ExcSocketHandle hSock) {
  if (!checkDevice(dev))
    return -1;
  else if (const EkaTcpSess *const s =
               dev->findTcpSess(hSock))
...
ExcUdpTxConnHandle
excUdpConnect(EkaDev *dev, EkaCoreId coreId,
  ...
  auto sessId = core->addUdpTxSess(srcMac, dstMac, srcIp,
