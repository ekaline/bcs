...
inline size_t pushFastCancelReport(
  ...
  epmReport->eventIsZero =
      hwEpmReport->event_indicator_is_zero;
...
std::pair<int, size_t> processFastCancelReport(
  ...
  case HwEpmActionStatus::Sent:
    ...
    EKA_LOG("Processgin HwEpmActionStatus::Sent, len=%d",
            srcReportLen);
    EKA_LOG(
        "FastCancelReport eventIsZero=%d, numInGroup=%d, "
        "transactTime=%ju, headerTime=%ju, seqNum=%u",
        hwEpmReport->event_indicator_is_zero,
        hwEpmReport->num_in_group,
        hwEpmReport->transact_time,
        hwEpmReport->header_time,
