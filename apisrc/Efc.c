...
EkaOpResult efcSetActionPayload(EkaDev *ekaDev,
  ...
  auto rc = epm->payloadHeapCopy(EFC_STRATEGY, payloadOffs,
                                 len, payload,
...
EkaOpResult
efcInitStrategy(EfcCtx *pEfcCtx,
 ...
 * that will fire if the opportunity should only only fired
 * on once. If this is < 0, then this will disable firing on
 * all cores.
 * @retval [See EkaOpResult].
 */
EkaOpResult efcEnableController(EfcCtx *pEfcCtx,
                                EkaCoreId primaryCoreId,
                                EfcArmVer ver) {
  if (pEfcCtx == NULL)
    on_error("pEfcCtx == NULL");
  ...
  if (primaryCoreId < 0)
    ...
    efc->armController(ver);

  /* assert (pEfcCtx != NULL); */
  /* download_conf2hw(pEfcCtx->dev); */
  /* if (primaryCoreId < 0) eka_arm_controller(pEfcCtx->dev,
   * 0); */
 ...
 *
 * @param pEfcCtx
 * @param pSecurityIds   This is a pointer to the first
...
efcEnableFiringOnSec(EfcCtx *pEfcCtx,
  ...
  if (pSecurityIds == NULL)
    on_error("pSecurityIds == NULL");
  ...
  uint64_t subscrCnt =
      (numSecurityIds << 32) | efc->numSecurities;
 ...
 * that we are setting.
 * @param pSecCtx         This is a pointer to the local
 * source SecCtx.
...
EkaOpResult efcSetStaticSecCtx(EfcCtx *pEfcCtx,
  ...
  if (pSecCtx == NULL)
    on_error("pSecCtx == NULL");

#if EFC_CTX_SANITY_CHECK
  if (static_cast<uint8_t>(efc->secIdList[hSecCtx] &
                           0xFF) !=
      pSecCtx->lowerBytesOfSecId)
    on_error("EFC_CTX_SANITY_CHECK error: "
             "secIdList[%jd] 0x%016jx & 0xFF != %02x",
             hSecCtx, efc->secIdList[hSecCtx],
             pSecCtx->lowerBytesOfSecId);
#endif
  ...
  /*   efc->ctxWriteBank[writeChan] * EKA_WORDS_PER_CTX_BANK
   * * 8; */
...
/**
 * This is just like setStaticSectx except it is for
 * SesCtxs.
 */
EkaOpResult efcSetSesCtx(EfcCtx *pEfcCtx,
                         ExcConnHandle hConn,
                         const SesCtx *pSesCtx) {
  on_error("This function is obsolete. Use "
           "efcSetFireTemplate()");
 ...
 * arrived on.
 * @param pEfcCtx
 * @param group       This is the multicast group that we
 * will be mapping from.
 * @param hConnection This is the ExcSessionId that we will
 * be mapping to.
 * @retval [See EkaOpResult].
 */
EkaOpResult efcSetGroupSesCtx(EfcCtx *pEfcCtx,
                              uint8_t group,
                              ExcConnHandle hConn) {
  if (!pEfcCtx)
    on_error("!pEfcCtx");
 ...
 *
 * @param pEfcCtx
 * @param pEfcRunCtx This is a pointer to the callbacks (and
