...
// #define EKA_SUBSCR_TABLE_ROWS (64*1024)
/* #endif */

#define EFC_SUBSCR_TABLE_ROWS_BITS 16
#define EFC_SUBSCR_TABLE_ROWS                              \
  (1 << EFC_SUBSCR_TABLE_ROWS_BITS)

// #define EKA_SUBSCR_TABLE_COLUMNS 42
#define EFC_SUBSCR_TABLE_COLUMNS 24

#define EKA_MAX_TCP_PACKET_SIZE 256 // 256B - HW Buf
#define EKA_MAX_TCP_PAYLOAD_SIZE                           \
  (EKA_MAX_TCP_PACKET_SIZE - 64) // 64B - NW headers

#define EKA_FH_SEC_HASH_LINES                              \
  (1024 * 128) // 17 LS bits (using 0x1FFFF mask)
#define EKA_FH_SEC_HASH_MASK                               \
  0x1FFFF // 17 LS bits to get the Hash table line

#define EKA_FH_GROUPS 64

#define ST_HASH_NUM_OF_BUCKETS 65536
#define EKA_FH_ORDERS_TO_ALLOCATE                          \
  (8 * 1024 * 1024) // orders per MC group
#define EKA_FH_ORDERS_HASH_MASK                            \
  0x00000000000FFFFF // 20 Low Bits to get to 1M indexes
#define EKA_FH_PLEVELS_TO_ALLOCATE                         \
  (4 * 1024 * 1024) // plevels per MC group

#define EKA_UCH_FASTPATH_ID                                \
  1 // User Channel ID for Fast Path and HW Fires
#define EKA_UCH_FIREREPORT_ID                              \
  0 // User Channel ID for Fast Path and HW Fires

#define HW_FEED_SHIFT_SIZE                                 \
  56 // position of the HW_FEED field in the FPGA VERSION
     // reg
#define HW_FEED_SHIFT_MASK                                 \
  0xf // 4 LS bits encode the HW_FEED
...
// constexpr double EKA_FPGA_FREQUENCY = 10.0 * 66 / 64 / 64
// * 1000;
...
#define SCRPAD_MC_STATE_SIZE                               \
  (UDP_CHANNELS * MAX_MC_GROUPS_PER_UDP_CH *               \
   sizeof(EkaMcState))
#define SCRPAD_EFC_SUBSCR_CNT                              \
  (SCRPAD_MC_STATE_BASE + SCRPAD_MC_STATE_SIZE)
...
#define ADDR_STATS_RX_PPS                                  \
  0xe0008 //  Statistics: 4 msb - max, 4 lsb - curr
#define ADDR_STATS_RX_BPS_MAX                              \
  0xe0010 //  Statistics: bytes per second max
#define ADDR_STATS_RX_BPS_CURR                             \
  0xe0018 //  Statistics: bytes per second current
#define ADDR_STATS_RX_BYTES_TOT                            \
  0xe0020 //  Statistics: total bytes
#define ADDR_STATS_RX_PKTS_TOT                             \
  0xe0028 //  Statistics: total packets

#define ADDR_STATS_TX_PPS                                  \
  0xe0040 //  Statistics: 4 msb - max, 4 lsb - curr
#define ADDR_STATS_TX_BPS_MAX                              \
  0xe0048 //  Statistics: bytes per second max
#define ADDR_STATS_TX_BPS_CURR                             \
  0xe0050 //  Statistics: bytes per second current
#define ADDR_STATS_TX_BYTES_TOT                            \
  0xe0058 //  Statistics: total bytes
#define ADDR_STATS_TX_PKTS_TOT                             \
  0xe0060 //  Statistics: total packets
