...
int getTradeTimeCb(const EfhDateComponents *,
                   uint32_t *iso8601Date, time_t *epochTime,
                   void *ctx) {
  return 0;
}

void *onOrder(const EfhOrderMsg *msg,
...
void *onTrade(const EfhTradeMsg *msg,
              EfhSecUserData secData,
              EfhRunUserData userData) {
  if (!testCtx->keep_work)
    return NULL;
...
void onException(EkaExceptionReport *msg,
...
void onFireReport(const void *fire_report_buf,
                  size_t size) {
  if (!testCtx->keep_work)
    return;
  printf("%s: Doing nothing \n", __func__);
...
void *onMd(const EfhMdHeader *msg,
           EfhRunUserData efhRunUserData) {
  if (!testCtx->keep_work)
    return NULL;
...
void *onQuote(const EfhQuoteMsg *msg,
              EfhSecUserData secData,
              EfhRunUserData userData) {
  if (!testCtx->keep_work)
    return NULL;
...
void *onImbalance(const EfhImbalanceMsg *msg,
                  EfhSecUserData secData,
                  EfhRunUserData userData) {
  return NULL;
}
...
void *onAuctionUpdate(const EfhAuctionUpdateMsg *msg,
                      EfhSecUserData secData,
                      EfhRunUserData userData) {
  if (!testCtx->keep_work)
    return NULL;
...
void *onFutureDefinition(const EfhFutureDefinitionMsg *msg,
                         EfhSecUserData secData,
                         EfhRunUserData userData) {
  if (!testCtx->keep_work)
    return NULL;
