...
bool EkaFhBxGr::parseMsg(
  ...
  auto genericHdr{
      reinterpret_cast<const BxFeed::GenericHdr *>(m)};
  ...
  case 'H': // TradingAction
    s = processTradingAction<FhSecurity,
                             BxFeed::TradingAction>(m);
    break;
    //--------------------------------------------------------------
  case 'a': // AddOrderShort
    s = processAddOrder<FhSecurity, BxFeed::AddOrderShort>(
        m);
    break;
    //--------------------------------------------------------------
  case 'A': // AddOrderLong
    s = processAddOrder<FhSecurity, BxFeed::AddOrderLong>(
        m);
    break;
    //--------------------------------------------------------------
  case 'j': // AddQuoteShort
    s = processAddQuote<FhSecurity, BxFeed::AddQuoteShort>(
        m);
    break;
    //--------------------------------------------------------------
  case 'J': // AddQuoteLong
    s = processAddQuote<FhSecurity, BxFeed::AddQuoteLong>(
        m);
    break;
    //--------------------------------------------------------------
  case 'E': // OrderExecuted
    ...
    s = processOrderExecuted<FhSecurity,
                             BxFeed::OrderExecuted, true>(
        m, sequence, msgTs, pEfhRunCtx);
    break;
    //--------------------------------------------------------------
  case 'C': // OrderExecutedPrice
    msgTs = BxFeed::getTs(m);
    s = processOrderExecuted<
        FhSecurity, BxFeed::OrderExecutedPrice, true>(
        m, sequence, msgTs, pEfhRunCtx);
    break;
    //--------------------------------------------------------------
  case 'X': // OrderCancel
    s = processOrderExecuted<FhSecurity,
                             BxFeed::OrderCancel, false>(
        m, 0, 0, nullptr);
    break;
    //--------------------------------------------------------------
  case 'u': // ReplaceOrderShort
    s = processReplaceOrder<FhSecurity,
                            BxFeed::ReplaceOrderShort>(m);
    break;
    //--------------------------------------------------------------
  case 'U': // ReplaceOrderLong
    s = processReplaceOrder<FhSecurity,
                            BxFeed::ReplaceOrderLong>(m);
    break;
    //--------------------------------------------------------------
  case 'D': // SingleSideDelete
    s = processDeleteOrder<FhSecurity,
                           BxFeed::SingleSideDelete>(m);
    break;
    //--------------------------------------------------------------
  case 'G': // SingleSideUpdate
    s = processSingleSideUpdate<FhSecurity,
                                BxFeed::SingleSideUpdate>(
        m);
    break;
    //--------------------------------------------------------------
  case 'k': // QuoteReplaceShort
    s = processReplaceQuote<FhSecurity,
                            BxFeed::QuoteReplaceShort>(m);
    break;
    //--------------------------------------------------------------
  case 'K': // QuoteReplaceLong
    s = processReplaceQuote<FhSecurity,
                            BxFeed::QuoteReplaceLong>(m);
    break;
    //--------------------------------------------------------------
  case 'Y': // QuoteDelete
    s = processDeleteQuote<FhSecurity, BxFeed::QuoteDelete>(
        m);
    break;
    //--------------------------------------------------------------
  case 'Q': // Trade for BX
    msgTs = BxFeed::getTs(m);
    s = processTrade<FhSecurity, BxFeed::Trade>(
        m, sequence, msgTs, pEfhRunCtx);
    break;
    //--------------------------------------------------------------
  case 'S': // SystemEvent
  ...
  case 'I': // NOII
    msgTs = BxFeed::getTs(m);
    processAuctionUpdate<FhSecurity, BxFeed::NOII>(
        m, sequence, msgTs, pEfhRunCtx);
    return false;
    //--------------------------------------------------------------
  case 'M': // EndOfSnapshot
    ...
    this->seq_after_snapshot =
        processEndOfSnapshot<BxFeed::EndOfSnapshot>(m, op);
    return true;
    //--------------------------------------------------------------
  case 'R': // Directory
    ...
    return false;
  default:
    on_error("UNEXPECTED Message type: enc=\'%c\', "
             "trackingNum=%d",
             enc, genericHdr->trackingNum);
  }
  if (!s)
    return false;
...
inline SecurityT *EkaFhBxGr::processOrderExecuted(
...
template <class SecurityT, class Msg>
inline SecurityT *EkaFhBxGr::processAuctionUpdate(
    const unsigned char *m, uint64_t sequence,
    uint64_t msgTs, const EfhRunCtx *pEfhRunCtx) {
  SecurityIdT securityId = getInstrumentId<Msg>(m);
  SecurityT *s = book->findSecurity(securityId);
  ...
  msg.quantity =
      getImbalanceSize<Msg>(m) + getPairedSize<Msg>(m);
  msg.price = getAuctionPrice<Msg>(m);
  msg.endTimeNanos =
      msgTs + getAuctionDurationNanos<Msg>(m);
...
template <class Msg>
inline void
EkaFhBxGr::processDefinition(const unsigned char *m,
                             const EfhRunCtx *pEfhRunCtx) {
  auto msg{reinterpret_cast<const Msg *>(m)};
