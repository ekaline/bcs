...
public:
  ...
  virtual bool parseMsg(const EfhRunCtx* pEfhRunCtx,const unsigned char* m,
  ...
  inline bool hasGapInGap(uint64_t sequence, uint msgInPkt) {
    ...
    if (sequence > expectedSeqGapInGap) {
      EKA_LOG("%s:%u: Gap-In-Gap at %s: expectedSeqGapInGap %ju != sequence %ju, "
	      "lost %ju",
	      EKA_EXCH_DECODE(exch),id,
	      printGrpState(),
  ...
  EkaDev*               dev                = NULL;
	...
	// 1s
	const uint64_t        StaleDataNanosecThreshold = 1'000'000'000; 
