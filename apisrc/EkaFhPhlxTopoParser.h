...
struct topo_generic_hdr { // only used to get nanoseconds
  char     message_type;         // 
  uint32_t time_nano;         // 4
} __attribute__((packed));


struct topo_time_stamp {
...
struct topo_option_directory { // TOPO OPTIONS
...
/* struct topo_option_directory { // TOPO ORDERS */
/*   char     message_type;         // 1 = 'D' */
/*   uint32_t time_seconds;         // 4 */
/*   uint32_t time_nano;            // 4 */
/*   uint32_t option_id;            // 4 */
...
struct topo_option_trading_action {
...
struct topo_option_security_open_closed {
...
struct topo_option_best_bid_and_ask_update_short {
...
struct topo_option_best_bid_and_ask_update_long {
  char     message_type;         // 1 = 'Q'
  uint32_t time_nano;            // 4
  uint32_t option_id;            // 4
  char     quote_condition;      // 1 
...
struct topo_option_best_bid_or_ask_update_short {
...
struct topo_option_best_bid_or_ask_update_long {
  char     message_type;         // 1 = 'A' or 'B'
				       // 'B' = Quote update bid side 
				       // 'A' = Quote update ask side
  uint32_t time_nano;            // 4
...
struct topo_option_trade_report {
...
struct topo_option_broken_trade_report {
