...
typedef union {
  ...
  struct {
    uint8_t reserved7 : 1;
    uint8_t originatedFromHw : 1; // should be added to
                                  // FastPathBytes
    uint8_t app_seq_inc : 1;
    uint8_t empty_report_en : 1;
    uint8_t feedbck_en : 1; // should HW send packet
                            // feedback to libary
    uint8_t action_valid : 1;
    uint8_t report_en : 1; // should HW send user
                           // application report
...
/* parameter EPM_ACTION_TYPE_SRC_ACTION = 1; //take size and
 * cs from action */
/* parameter EPM_ACTION_TYPE_SRC_DESC   = 2; //take size and
 * cs from desc */

enum class TcpCsSizeSource : uint8_t {
  FROM_ACTION = 1,
  FROM_DESCR = 2
};
...
/* FPGA code: */
/* typedef struct packed { */
/*         bit [7:0]  islocal;             ///< True ->
 * called from epmRaiseTrigger */
/*         bit [63:0] user;              ///< Opaque value
 * copied from EpmAction */
/*         bit [15:0] postStratEnable;   ///< Strategy-level
 * enable bits after fire */
/* 	bit [15:0] preStratEnable;    ///< Strategy-level
 * enable bits before fire */
/* 	bit [15:0] postLocalEnable;   ///< Action-level
 * enable bits after firing */
/* 	bit [15:0] preLocalEnable;    ///< Action-level
 * enable bits before fire */
/*         bit [7:0]  error;             ///< Error code for
 * SendError */
/*         bit [7:0]  action;            ///< What device
 * did in response to trigger */
/* 	bit [7:0]  strategyid; */
...
/* parameter EKA_ACTIONRESULT_Unknown          = 0;  ///<
 * Zero initialization yields an invalid value */
/* parameter EKA_ACTIONRESULT_Sent             = 1;  ///<
 * All action payloads sent successfully */
/* parameter EKA_ACTIONRESULT_InvalidToken     = 2;  ///<
 * Trigger security token did not match action token */
/* parameter EKA_ACTIONRESULT_InvalidStrategy  = 3;  ///<
 * Strategy id was not valid */
/* parameter EKA_ACTIONRESULT_InvalidAction    = 4;  ///<
 * Action id was not valid */
/* parameter EKA_ACTIONRESULT_DisabledAction   = 5;  ///<
 * Did not fire because an enable bit was not set */
/* parameter EKA_ACTIONRESULT_SendError        = 6;  ///<
 * Send error occured (e.g., TCP session closed) */
/* parameter EKA_ACTIONRESULT_HWPeriodicStatus = 255; ///<
 * HW generated periodic status */
