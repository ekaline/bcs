...
// Parameter EPM_SIZE_TCP_CHCK_SUM bound to: 2 - type:
// integer
...
public:
  EpmHwFieldsMap() {
    ...
    for (uint8_t i = 0; i < (const int)HwField::Count;
         i++) {
      map[i].fIdx = (HwField)i;
      strcpy(map[i].name, EpmHwField2Str(map[i].fIdx));
      map[i].start =
          i == 0 ? 0 : map[i - 1].start + map[i - 1].size;
    }
  ...
  void print() {
    for (uint8_t i = 0; i < (const int)HwField::Count;
         i++) {
      EKA_LOG("%2u: %20s %3d %3d", (uint)map[i].fIdx,
              map[i].name, map[i].start, map[i].size);
    }
  }
