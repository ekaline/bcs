...
static inline bool isPreTradeTime(int hour, int minute) {
  ...
  if (ct->tm_hour < hour ||
			(ct->tm_hour == hour && ct->tm_min < minute))
		return true;
  return false;
}
...
EkaOpResult EkaFhPhlxTopo::runGroups( EfhCtx* pEfhCtx,
  ...
  while (runGr->thread_active && ! runGr->stoppedByExchange) {
    ...
    if (gr->state == EkaFhGroup::GrpState::NORMAL && 
      ...
      EKA_WARN("%s:%u: TEST GAP INJECTED: "
							 ...
							 "%ju with unknown number of messages dropped",
    ...
    case EkaFhGroup::GrpState::SNAPSHOT_GAP : {
      if (gr->gapClosed || sequence <= gr->recovery_sequence) {
				...
				gr->expected_sequence = msgInPkt == 0 ?
					gr->recovery_sequence : gr->recovery_sequence + 1;
				...
				EKA_LOG("%s:%u: SNAPSHOT GAP closed: "
								"recovery_sequence %ju >= sequence %ju, "
								"expected_sequence=%ju, msgInPkt=%u",
								EKA_EXCH_DECODE(gr->exch),gr->id,
								gr->recovery_sequence,sequence,
								gr->expected_sequence,msgInPkt);
