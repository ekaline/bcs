...
typedef int (*EfhGetTradeTimeFn)(const EfhDateComponents *,
...
struct EfhInitCtx {
  ...
  EfhGetTradeTimeFn
      getTradeTime; // Used to map a CME-style expiration to
                    // a trading date/time
  void *getTradeTimeCtx;    // Context object passed into
                            // getTradeTime
  bool printParsedMessages; // used for Ekaline internal
                            // testing

  /* This is true if we expect to receive marketdata updates
   * when we run efhRun().  If this is false, we dont expect
   * updates, and so Ekaline can save memory and avoid
   * creating structures that arent needed.  */
  bool recvSoftwareMd;

  /* Used for Ekaline lab tests only */
  bool subscribe_all;

  /* This is true if we expect to receive raw relevant
   * market data for EFC testing. noTob = false for regular
   * EFH use   */
  bool noTob = false;
};
 ...
 *
 * @oaram ppEfhCtx This is an output parameter that will
 * contain an initialized EfhCtx* if this function is
 * successful.
 * @param ekaDev   EkaDev of the device used to host the
 * feed handler
 * @param initCtx  This is a list of key value parameters
 * that replaces the old eka_conf values that were passed in
 * the old api.
 * @return        This will return an appropriate
 * EkalineOpResult indicating success or an error code.
 */
EkaOpResult efhInit(EfhCtx **ppEfhCtx, EkaDev *ekaDev,
                    const EfhInitCtx *initCtx);
 ...
 */
EkaOpResult efhGetDefs(EfhCtx *efhCtx,
                       const struct EfhRunCtx *efhRunCtx,
...
EkaOpResult efhSubscribeStatic(
    EfhCtx *efhCtx, const EkaGroup *group,
 ...
 * Call this function when you are done with all your
 * efhSubscribeStatic() calls.
 */
...
/**
 * EfhMd functionality allows to get callback on every raw
 * market data event
 *
 */
...
typedef void *(*onEfhMdMsgCb)(
    const EfhMdHeader *msg, EfhRunUserData efhRunUserData);
...
#define _DECL_CALLBACK(_msg, ...)                          \
  void *(EKA__DELAYED_CAT3(*onEfh, _msg, MsgCb))(          \
      const EKA__DELAYED_CAT3(Efh, _msg, Msg) * msg,       \
      EfhSecUserData efhSecUserData,                       \
      EfhRunUserData efhRunUserData);
  EfhMsgType_ENUM_ITER(_DECL_CALLBACK)

      void (*onEkaExceptionReportCb)(
          EkaExceptionReport *msg,
          EfhRunUserData efhRunUserData);
...
/**
 * This function will run the ekaline Fh on the current
 * thread and make callbacks for messages processed. This
 * function should not return until we shut the Ekaline
 * system down via ekaClose().
 *
 * @param pEfhCtx
 ...
 * @retval [See EkaOpResult].
 */
EkaOpResult efhRunGroups(EfhCtx *efhCtx,
...
EkaOpResult efhMonitorFhGroupState(EfhCtx *efhCtx,
                                   EkaGroup *group,
                                   bool verbose = false);

/**
 * This will destroy an Ekaline feedhandler created with
 * efhInit.
 *
 * @oaram efhCtx  An initialized EfhCtx* obtained from
 * efhInit.
 * @return        This will return an appropriate
 * EkalineOpResult indicating success or an error code.
 */
