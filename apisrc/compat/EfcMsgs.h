/*
 * EfcMsgs.h
 *     This header file contains the api for Ekaline firing
 * controller messages.
 */
...
#define EfcReportType_ENUM_ITER(_x)                        \
  _x(ControllerState, 1000) _x(MdReport, 2000)             \
      _x(SecurityCtx, 3000) _x(ExceptionReport, 4000)      \
          _x(FirePkt, 5000) _x(EpmReport, 6000)            \
...
#define EfcReportHdr_FIELD_ITER(_x)                        \
  _x(EfcReportType,                                        \
     type) /* which report out of num_of_reports (counting \
              from 1) */                                   \
      _x(uint8_t,                                          \
         idx) /* size of the body (= number of bytes after \
...
#define EfcControllerState_FIELD_ITER(_x)                  \
  ...
  _x(uint8_t, unarm_reason) _x(uint8_t, fire_reason)
  EfcControllerState_FIELD_ITER(EKA__FIELD_DEF)
} EfcControllerState; // single appearence

typedef struct {
#define EfcMdReport_FIELD_ITER(_x)                         \
  _x(uint64_t, timestamp) _x(uint64_t, sequence)           \
      _x(uint8_t, side) _x(uint64_t, price)                \
          _x(uint64_t, size) _x(uint64_t, security_id)     \
              _x(uint8_t, group_id) _x(uint8_t, core_id)
...
#define EfcFireReport_FIELD_ITER(_x)                       \
  _x(uint8_t, reason) _x(uint64_t, size) _x(uint64_t,      \
                                            price)         \
      _x(uint64_t, security_id) /** = core * 128 + sess */ \
      _x(uint16_t,                                         \
         session) /** HW timestamp of the first fire */    \
...
#define EfcCommonSessionCtx_FIELD_ITER(_x)                 \
  /* = core * 128 + sess */                                \
  _x(uint16_t, fired_session) _x(uint64_t, cl_ord_id)
