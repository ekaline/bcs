...
void EkaFhGroup::sendFeedUp(const EfhRunCtx* pEfhRunCtx) {
  ...
  pEfhRunCtx->onEfhGroupStateChangedMsgCb(&msg, 0,
...
void EkaFhGroup::sendFeedDownStaleData(const EfhRunCtx* pEfhRunCtx) {
  if (pEfhRunCtx == NULL)
		on_error("pEfhRunCtx == NULL");
  if (pEfhRunCtx->onEfhGroupStateChangedMsgCb == NULL)
    on_error("pEfhRunCtx->onEfhGroupStateChangedMsgCb == NULL");
...
void EkaFhGroup::sendBackInTimeEvent(const EfhRunCtx* pEfhRunCtx,
...
void
EkaFhGroup::sendRetransmitExchangeError(const EfhRunCtx* pEfhRunCtx,
																				bool sleepAfterCb) {
  if (pEfhRunCtx == NULL)
		on_error("pEfhRunCtx == NULL");
  if (pEfhRunCtx->onEfhGroupStateChangedMsgCb == NULL)
...
void
EkaFhGroup::sendRetransmitSocketError(const EfhRunCtx* pEfhRunCtx,
																			bool sleepAfterCb) {
  if (pEfhRunCtx == NULL)
		on_error("pEfhRunCtx == NULL");
  if (pEfhRunCtx->onEfhGroupStateChangedMsgCb == NULL)
...
int EkaFhGroup::init (EfhCtx* _pEfhCtx, 
  ...
  if (pInitCtx->printParsedMessages) {
    ...
    if((parser_log = fopen(parsedMsgFileName.c_str(),"w")) == NULL)
			on_error ("Error %s",parsedMsgFileName.c_str());
    EKA_LOG("%s:%u created file %s",
						EKA_EXCH_DECODE(exch),id,
						parsedMsgFileName.c_str());
  }
...
int EkaFhGroup::credentialAcquire(EkaCredentialType credType,
	...
	if (rc != 0) 
		on_error("%s:%u Failed to credAcquire for \'%.*s\' rc=%d \'%s\'",
						 EKA_EXCH_DECODE(exch),id,
						 (int)credNameSize,credName,rc,strerror(errno));
	return rc;
}
/* ############################################# */
