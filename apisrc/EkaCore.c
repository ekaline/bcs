...
EkaCore::EkaCore(EkaDev *pEkaDev, uint8_t lane, uint32_t ip,
  ...
  isTcpCore = dev->ekaHwCaps->hwCaps.core.bitmap_tcp_cores &
              (1 << coreId);

  EKA_LOG("%s Core %u: %s %s",
          isTcpCore ? "TCP Enabled" : "NON TCP", coreId,
          EKA_MAC2STR(macSa), EKA_IP2STR(srcIp));
...
bool EkaCore::initTcp() {
  if (!isTcpCore) {
    on_error("TCP functionality is disabled for lane %u",
             coreId);
    return false;
  }
  ...
  if (gwIp != 0)
    setNetifGw(dev, coreId, &gwIp);
  if (netmask != 0)
    setNetifNetmask(dev, coreId, &netmask);
  if (!eka_is_all_zeros(&macDa, 6)) {
    if (ekaAddArpEntry(dev, coreId, &gwIp, macDa) == -1)
      on_error("Failed to add static ARP entry for lane "
               "%u: %s --> %s",
               coreId, EKA_IP2STR(gwIp),
               EKA_MAC2STR(macDa));
    EKA_LOG("Lane %u: Adding GW static ARP: %s", coreId,
            EKA_MAC2STR(macDa));
  }

  tcpSess[CONTROL_SESS_ID] = new EkaTcpSess(
      dev, this, coreId, CONTROL_SESS_ID, 0 /* srcIp */,
      0 /* dstIp */, 0 /* dstPort */, macSa);
  ...
  EKA_LOG(
      "Enabling TCP RX on core %u for MAC %s (0x%016jx)",
      coreId, EKA_MAC2STR(macSa), be64toh(macSaBuf));

  eka_write(dev, 0xe0000 + coreId * 0x1000 + 0x200,
            be64toh(macSaBuf));
...
EkaCore::~EkaCore() {
  ...
  /* if (feedServerUdpChannel != NULL) delete
   * feedServerUdpChannel; */
}

/* -------------------------------------------------------------
 */
EkaTcpSess *EkaCore::findTcpSess(uint32_t ipSrc,
                                 uint16_t srcPort,
                                 uint32_t ipDst,
                                 uint16_t dstPort) {
  for (uint i = 0; i < MAX_SESS_PER_CORE; i++) {
    if (tcpSess[i] == NULL)
      continue; // on_error("Core %u, tcpSess[%u] ==
                // NULL",coreId,i);
    if (tcpSess[i]->myParams(ipSrc, srcPort, ipDst,
                             dstPort))
      return tcpSess[i];
  }
  return NULL;
...
/* -------------------------------------------------------------
 */
/* EkaUdpSess* EkaCore::findUdpSess(uint32_t ip, uint16_t
 * port) { */
/*   for (uint i = 0; i < udpSessions; i++) { */
/*     if (udpSess[i] == NULL) on_error("Core %u,
 * udpSess[%u] == NULL",coreId,i); */
...
uint8_t EkaCore::getFreeTcpSess() {
  ...
  on_error("No free Tcp Sessions to allocate (all %u are "
           "in use)",
           MAX_SESS_PER_CORE);
  return 0xFF;
}
...
uint EkaCore::addTcpSess() {
  ...
  tcpSess[sessId] = new EkaTcpSess(
      dev, this, coreId, sessId, srcIp, 0, 0, macSa);
  if (!tcpSess[sessId])
    on_error("Failed creating tcpSess[%u]", sessId);
  EKA_LOG("Created Session %u on Core %u: "
          "macsa=%s, srcIp=%s",
          sessId, coreId, EKA_MAC2STR(macSa),
          EKA_IP2STR(srcIp));
  return sessId;
}
/* -------------------------------------------------------------
 */
uint EkaCore::addUdpTxSess(eka_ether_addr srcMac,
  ...
  udpTxSess[sessId] = new EkaUdpTxSess(
      dev, coreId, sessId, srcMac, dstMac, srcIp, dstIp,
...
void EkaCore::suppressOldTcpSess(uint8_t newSessId,
  ...
  for (uint i = 0; i < MAX_SESS_PER_CORE; i++) {
    ...
    if (tcpSess[i]->srcIp == srcIp &&
      ...
      delete tcpSess[i];
      tcpSess[i] = NULL;
      EKA_LOG("Session %u:%u is suppressed %s:%u --> %s:%u",
              coreId, i, EKA_IP2STR(srcIp), srcPort,
...
/*   uint8_t sessId = getFreeTcpSess(); */
/*   tcpSessions++; */
/*   EKA_LOG("Opening TCP session %u from core %u, srcIp =
 * %s",sessId,coreId,EKA_IP2STR(srcIp)); */

/*   tcpSess[sessId] = new EkaTcpSess(dev, this, coreId,
 * sessId, srcIp, dstIp, dstPort, macSa); */
/*   tcpSess[sessId]->bind(); */
 ...
 * on_error("udpSessions = %u",tcpSessions); */
/*   uint8_t sessId = udpSessions++; */
/*   udpSess[sessId] = new EkaUdpSess(dev, sessId, ip,
 * port); */
/*   return sessId; */
/* } */
