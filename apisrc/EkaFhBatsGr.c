...
bool EkaFhBatsGr::processUdpPkt(
  ...
  for (uint msg = 0; msg < msgInPkt; msg++) {
    ...
    //---------------------------------------------------
    if (parseMsg(pEfhRunCtx, msgData, sequence,
                 EkaFhMode::MCAST, startTime))
      return true;
    //---------------------------------------------------
...
void EkaFhBatsGr::pushUdpPkt2Q(const uint8_t *pkt,
  ...
  for (uint msg = 0; msg < msgInPkt; msg++) {
    ...
    if (msg_len > fh_msg::MSG_SIZE)
      on_error("msg_len %u > fh_msg::MSG_SIZE %jd", msg_len,
               fh_msg::MSG_SIZE);
    fh_msg *n = q->push();
...
/* ##################################################### */
int EkaFhBatsGr::closeSnapshotGap(
  ...
  std::string threadName =
      ...
      std::to_string(id);
  auto attr = new EkaFhThreadAttr(pEfhCtx, pEfhRunCtx, this,
                                  startSeq, endSeq,
                                  EkaFhMode::SNAPSHOT);
  if (!attr)
    on_error("attr = NULL");
  ...
  dev->createThread(
      ...
      getSpinData, attr, dev->createThreadContext,
      (uintptr_t *)&snapshot_thread);

  return 0;
}
/* ##################################################### */
int EkaFhBatsGr::closeIncrementalGap(
    EfhCtx *pEfhCtx, const EfhRunCtx *pEfhRunCtx,
    uint64_t startSeq, uint64_t endSeq) {
  ...
  dev->createThread(
      threadName.c_str(), EkaServiceType::kFeedSnapshot,
      getGrpRetransmitData, attr, dev->createThreadContext,
      (uintptr_t *)&snapshot_thread);
